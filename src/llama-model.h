...
struct llama_model {
  ...
  int main_gpu;
  int max_gpu =
      0; // max. number of GPUs to use per layer for aplit mode "graph"
  int n_gpu_layers;
  int n_split_layers = -1; // number of layers to graph-split (-1 = all)
  ...
  struct layer_buft {
    ...
    layer_buft(ggml_backend_buffer_type_t matrix)
        : buft_matrix(matrix), buft(matrix) {}
    layer_buft(ggml_backend_buffer_type_t matrix,
               ggml_backend_buffer_type_t other)
        : buft_matrix(matrix), buft(other) {}

    ggml_backend_buffer_type_t
        buft_matrix; // matrices only - used by split buffers and backends that
                     // support only matrix multiplication
  ...
  bool has_tensor_overrides() const { return tensor_overrides; }
...
struct llama_lora_weight {
  ...
  llama_lora_weight(struct ggml_tensor *a, struct ggml_tensor *b)
      : a(a), b(b) {}
};
...
struct LLM_TN {
  ...
  std::string operator()(llm_tensor tensor, const std::string &suffix, int bid,
                         int xid) const;
};
