...
void llama_decode_reset() { stop_internal_decode = false; }

void llama_decode_stop() { stop_internal_decode = true; }
...
static std::vector<rpc_device>
extract_device_from_rpc_device(std::vector<std::string> devices) {
...
static std::string gguf_data_to_str(enum gguf_type type, const void *data,
                                    int i) {
  switch (type) {
...
ggml_backend_buffer_type_t llama_default_buffer_type_cpu(bool host_buffer) {
  ...
  // host buffers should only be used when data is expected to be copied to/from
  // the GPU
  if (host_buffer) {
...
static const char *
llama_expert_gating_func_name(llm_expert_gating_func_type type) {
  switch (type) {
...
llama_default_buffer_type_offload(const llama_model &model, int gpu) {
  ggml_backend_buffer_type_t buft = nullptr;
...
static ggml_backend_buffer_type_t
llama_default_buffer_type_split(const llama_model &model, int fallback_gpu) {
  ggml_backend_buffer_type_t buft = nullptr;
...
int llama_model::device_count() const { return llama_get_device_count(*this); }

ggml_backend_buffer_type_t
llama_model::default_buffer_type_offload(int device) const {
...
bool llama_context::can_reuse_graph(const llama_batch &u_batch) {
  ...
  if (!cparams.graph_reuse)
    return false;
  return u_batch.all_seq_id == prev->all_seq_id && kv_self.head > 0 &&
         kv_self.n == prev->n_kv && n_outputs == prev->n_outputs &&
         update_cache_copies();
}

bool llama_context::update_cache_copies() {
  int n_layer = model.hparams.n_layer -
                model.hparams.nextn_predict_layers; // cache_copies.size()/2;
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    if (kl) {
      GGML_ASSERT(model.split_mode == LLAMA_SPLIT_MODE_GRAPH ||
                  model.split_mode == LLAMA_SPLIT_MODE_ATTN);
      GGML_ASSERT(model.splits.size() > 1);
      auto vl = !kv_self.v_l.empty() && kv_self.v_l[il]
                    ? (ggml_split_tensor_t *)kv_self.v_l[il]->extra
                    : nullptr;
      ...
      for (int id = 0; id < kl->n_device; ++id) {
        ...
        if (!c.cpy || c.cpy->op != GGML_OP_CPY ||
            c.cpy->view_src != kl->splits[id])
          return false;
...
llama_context::llama_context(const llama_model &model)
    : model(model), sampling(llama_n_vocab(&model)),
...
static bool llama_kv_cache_init(struct llama_kv_cache &cache,
                                const llama_context *ctx, ggml_type type_k,
                                ggml_type type_v, uint32_t kv_size,
                                bool offload) {
  ...
  if ((model.split_mode == LLAMA_SPLIT_MODE_GRAPH ||
       model.split_mode == LLAMA_SPLIT_MODE_ATTN) &&
      model.arch != LLM_ARCH_DEEPSEEK2 && offload) {
  ...
  for (auto &it : buft_layer_count) {
    ...
    if (split_cache)
      ctx_mem_size +=
          2 * model.splits.size() * n_layers * ggml_tensor_overhead();
  ...
  if (model.arch == LLM_ARCH_DEEPSEEK2) {
    ...
    if (!have_wkv_b) {
      if (cparams.mla_attn != 1) {
        ...
        if (cparams.mla_attn > 1) {
          LLAMA_LOG_WARN(
              "%s: ** Prompt processing performance will be crippled **\n",
              __func__);
        }
        LLAMA_LOG_WARN(
            "=========================================================\n");
  ...
  for (int i = 0; i < (int)n_layer; i++) {
    const uint32_t n_embd_v_gqa =
        hparams.n_embd_v_gqa(i) + hparams.n_embd_v_s();
    const uint32_t n_head_kv = hparams.n_head_kv(i);
    const uint32_t n_embd_head_k = hparams.n_embd_head_k;

    struct ggml_context *ctx = split_cache
                                   ? ctx_map.at(model.buft_layer[i].buft_matrix)
                               : offload ? ctx_map.at(model.buft_layer[i].buft)
                                         : cache.ctxs.front();
    ggml_tensor *k;
    ggml_tensor *v;
    if (model.arch == LLM_ARCH_DEEPSEEK2 && cparams.mla_attn) {
      ...
      // LLAMA_LOG_INFO("%s: layer %d: n_embd_head_qk_rope = %d, kv_lora_rank =
      // %d\n", __func__, i, n_embd_head_qk_rope, kv_lora_rank);
      if (cparams.flash_attn) {
        ggml_tensor *kv = ggml_new_tensor_2d(
            ctx, cache.type_k, kv_lora_rank + n_embd_head_qk_rope, kv_size);
        ggml_format_name(kv, "cache_k_l%d", i);
        cache.k_l.push_back(kv);
      } else {
        auto kv_type = cparams.mla_attn == 1 ? cache.type_k : cache.type_v;
        ggml_tensor *kv = ggml_new_tensor_2d(
            ctx, kv_type, kv_lora_rank + n_embd_head_qk_rope, kv_size);
        ggml_format_name(kv, "cache_k_l%d", i);
        cache.k_l.push_back(kv);
        if (cparams.mla_attn == 1) {
          ggml_tensor *kvt =
              ggml_new_tensor_1d(ctx, cache.type_v, kv_lora_rank * kv_size);
      ...
      if (split_cache_i) {
        bool use_V_for_K =
            model.layers[i].attn_k_norm &&
                    model.layers[i].attn_k_norm->ne[0] == K->ne[1]
                ? true
                : false;
        ...
        for (int is = 0; is < extra_K->n_device; ++is) {
          ...
          if (!split)
            continue;
          int nhead_kv = use_V_for_K ? split->ne[1] / n_embd_head_v
                                     : split->ne[1] / n_embd_head_k;
          if (use_V_for_K) {
            LLAMA_LOG_DEBUG("K_cache(%d, %d): using %d instead of %ld heads\n",
                            i, is, nhead_kv,
                            extra_K->splits[is]->ne[1] / n_embd_head_k);
          }
          split_k_l.tensor_splits[is] = ggml_new_tensor_2d(
              ctx, type_k, n_embd_head_k, nhead_kv * kv_size);
        ...
        for (int is = 0; is < extra_V->n_device; ++is) {
          ...
          if (!split)
            continue;
          split_v_l.tensor_splits[is] =
              ggml_new_tensor_1d(ctx, type_v, split->ne[1] * kv_size);
  ...
  if (model.arch == LLM_ARCH_DEEPSEEK2 && cparams.mla_attn && n_mla < n_layer &&
      n_mla > 0) {
    LLAMA_LOG_ERROR("%s: unexpected situation with %d out of %d layers having "
                    "MLA enabled\n",
  ...
  for (auto it : ctx_map) {
    ...
    int ntensor = 0;
    for (auto t = ggml_get_first_tensor(ctx); t != NULL;
  ...
  if (split_cache) {
    ...
    for (int i = 0; i < int(mem_split.size()); ++i)
      printf("    Device %d:  %g MiB\n", i, mem_split[i] / 1024. / 1024.);
  }
      ...
      for (int32_t j = 0; j < batch.n_seq_id[i]; ++j) {
        ...
        if ((uint32_t)seq_id < cache.size) {
          ...
          if (batch.pos[i] != cache.cells[seq_id].pos + 1) {
            ...
            // Clearing the state mid-batch would require special-casing which
            // isn't done.
            LLAMA_LOG_WARN("%s: non-consecutive token position %d after %d for "
                           "sequence %d\n",
                           __func__, batch.pos[i], cache.cells[seq_id].pos,
                           seq_id);
          }
          ...
          // NOTE: seq_ids are not inserted here; they are handled when the
          // input tensors are set
        } else {
          // too big seq_id
          // TODO: would it be possible to resize the KV cache size instead?
          LLAMA_LOG_ERROR("%s: seq_id=%d >= kv_size=%d Try using a bigger "
                          "--parallel value\n",
...
static bool llama_kv_cache_seq_rm(struct llama_kv_cache &cache,
  ...
  if (cache.recurrent) {
    ...
    if (0 <= seq_id) {
      ...
      if ((0 < p0 && p0 <= cache.cells[seq_id].pos) ||
          (0 < p1 && p1 <= cache.cells[seq_id].pos)) {
        return false;
      }
    } else {
...
static void llama_kv_cache_seq_cp(struct llama_kv_cache &cache,
  ...
  if (cache.recurrent) {
    if ((uint32_t)seq_id_dst < cache.size &&
...
static void llama_kv_cache_seq_add(struct llama_kv_cache &cache,
                                   llama_seq_id seq_id, llama_pos p0,
...
static void llama_kv_cache_seq_div(struct llama_kv_cache &cache,
                                   llama_seq_id seq_id, llama_pos p0,
                                   llama_pos p1, int d) {
  if (p0 < 0)
    p0 = 0;
...
static llama_pos llama_kv_cache_seq_pos_max(struct llama_kv_cache &cache,
                                            llama_seq_id seq_id) {
  llama_pos result = 0;
...
static uint32_t
llama_kv_cache_get_padding(const struct llama_cparams &cparams) {
...
static void llm_load_print_meta(llama_model_loader &ml, llama_model &model) {
  ...
  const char *rope_scaling_type =
      hparams.rope_scaling_type_name(hparams.rope_scaling_type_train);
  ...
  if (!hparams.vocab_only) {
    ...
    LLAMA_LOG_INFO("%s: n_head           = %s\n", __func__,
                   print_f([&](uint32_t il) { return hparams.n_head(il); },
                           hparams.n_layer)
                       .c_str());
    LLAMA_LOG_INFO("%s: n_head_kv        = %s\n", __func__,
                   print_f([&](uint32_t il) { return hparams.n_head_kv(il); },
                           hparams.n_layer)
                       .c_str());
    LLAMA_LOG_INFO("%s: n_rot            = %u\n", __func__, hparams.n_rot);
    LLAMA_LOG_INFO("%s: n_swa            = %u\n", __func__, hparams.n_swa);
    LLAMA_LOG_INFO("%s: n_swa_pattern    = %u\n", __func__,
                   hparams.n_swa_pattern);
    ...
    LLAMA_LOG_INFO(
        "%s: n_gqa            = %s\n", __func__,
        print_f([&](uint32_t il) { return hparams.n_gqa(il); }, hparams.n_layer)
            .c_str());
    LLAMA_LOG_INFO(
        "%s: n_embd_k_gqa     = %s\n", __func__,
        print_f([&](uint32_t il) { return hparams.n_embd_k_gqa(il); },
                hparams.n_layer)
            .c_str());
    LLAMA_LOG_INFO(
        "%s: n_embd_v_gqa     = %s\n", __func__,
        print_f([&](uint32_t il) { return hparams.n_embd_v_gqa(il); },
                hparams.n_layer)
            .c_str());
    ...
    LLAMA_LOG_INFO(
        "%s: n_ff             = %s\n", __func__,
        print_f([&](uint32_t il) { return hparams.n_ff(il); }, hparams.n_layer)
            .c_str());
  ...
  }
  {
    ...
    if (meta_tke && meta_out) {
      ...
      if (n_bytes < GiB) {
        LLAMA_LOG_INFO("%s: repeating layers = %.3f MiB (%.3f BPW", __func__,
                       n_bytes / 1024.0 / 1024.0, n_bytes * 8.0 / n_elements);
      } else {
        LLAMA_LOG_INFO("%s: repeating layers = %.3f GiB (%.3f BPW", __func__,
                       n_bytes / 1024.0 / 1024.0 / 1024.0,
  ...
  if (model.arch == LLM_ARCH_DEEPSEEK2) {
    ...
    LLAMA_LOG_INFO("%s: n_expert_shared      = %d\n", __func__,
                   hparams.n_expert_shared);
    LLAMA_LOG_INFO("%s: expert_weights_scale = %.1f\n", __func__,
                   hparams.expert_weights_scale);
    LLAMA_LOG_INFO("%s: expert_weights_norm  = %d\n", __func__,
                   hparams.expert_weights_norm);
    LLAMA_LOG_INFO(
        "%s: expert_gating_func   = %s\n", __func__,
  ...
  if (model.arch == LLM_ARCH_BAILINGMOE2) {
    ...
    LLAMA_LOG_INFO("%s: n_expert_groups      = %d\n", __func__,
                   hparams.n_expert_groups);
    LLAMA_LOG_INFO("%s: n_group_used         = %d\n", __func__,
                   hparams.n_group_used);
    LLAMA_LOG_INFO("%s: expert_weights_scale = %.1f\n", __func__,
                   hparams.expert_weights_scale);
    LLAMA_LOG_INFO("%s: expert_weights_norm  = %d\n", __func__,
                   hparams.expert_weights_norm);
...
static void llm_prepare_mla(llama_model &model, int mla) {
  ...
  if (mla > 0 && n_to_compute > 0) {
    // Prepare wk_b tensors to enable MLA usage also for model files that do not
    // include the wk_b tensors (because, e.g., they were converted using
    // mainline llama.cpp) We do it here because otherwise wkv_b may get
    // run-time-repacked, which will make preparation of wk_b impossible. It
    // also has the benefit that wk_b will get automatically run-time repacked
    // if the rtr option is set. The downside is that we will prepare wk_b even
    // if it is not needed (because MLA is not being used). If we wanted to
    // avoid computing wk_b from wkv_b if not needed, we would need to propagate
    // the context parameters to the model loading function. On the other hand,
    // in some hypothetical bright future, where we are able to use the optimum
    // settings for the computation, which for DeepSeekV3/R1/Lite is no MLA + FA
    // for prompt processing, and MLA + FA for token generation, it would be
    // useful to change the MLA setting on the fly, depending on context. In
    // that case, having prepared the MLA tensors here is the right ting to
    // do^TM.
    ...
    for (int il = 1; il < n_layer; ++il) {
      // Somehow the number of heads is being defined as being per layer. Not
      // sure why this is the case, but for now we do not support strange models
      // that have different numbers of heads in different model layers.
      if ((int)hparams.n_head(il) != n_head)
    ...
    for (auto &l : model.layers) {
      if (!l.wk_b) {
        auto new_type =
            ggml_is_quantized(l.wkv_b->type) ? GGML_TYPE_Q8_0 : l.wkv_b->type;
        auto size = ggml_row_size(new_type, n_embd_head_qk_nope) *
                    kv_lora_rank * n_head;
    ...
    auto context_size = max_wk_size + 2 * n_embd_head_qk_nope * kv_lora_rank *
                                          n_head * sizeof(float);
    context_size *= 2; // just in case;
    std::vector<uint8_t> wkv_buffer;
    if (max_wkv_size > 0)
      wkv_buffer.resize(max_wkv_size);
    // So, transposing tensors and then making them contiguous as needed for
    // wk_b may or may not be supported on all backends. Hence, to be sure that
    // the preparation of wk_b will work correctly, we do it on the CPU backend.
    // We then copy the resulting tensor data to the bacikend where wkv_b is
    // stored.
    ggml_init_params params{context_size, nullptr, true};
    auto ctx = ggml_init(params);
    auto graph = ggml_new_graph_custom(ctx, 8, false);
    std::vector<uint8_t> tensor_data(2 * n_embd_head_qk_nope * kv_lora_rank *
                                         n_head * sizeof(float) +
                                     max_wk_size);
    for (int il = 0; il < n_layer; ++il) {
      ...
      auto wkv_b = *l.wkv_b;
      if (!ggml_backend_buffer_is_host(l.wkv_b->buffer)) {
        ggml_backend_tensor_get(l.wkv_b, wkv_buffer.data(), 0,
                                ggml_nbytes(l.wkv_b));
        wkv_b.data = wkv_buffer.data();
      }
      auto wk_b_view = ggml_view_3d(
          ctx, &wkv_b, kv_lora_rank, n_embd_head_qk_nope, n_head,
          l.wkv_b->nb[1],
          l.wkv_b->nb[1] * (n_embd_head_qk_nope + n_embd_head_v), 0);
      ...
      auto new_type = ggml_is_quantized(wkv_b.type)
                          ? wkv_b.type >= GGML_TYPE_Q4_0_R8 &&
                                    wkv_b.type <= GGML_TYPE_Q8_K_R8
                                ? GGML_TYPE_Q8_0_R8
                                : GGML_TYPE_Q8_0
                          : wkv_b.type;
      ...
      auto plan =
          ggml_graph_plan(graph, std::thread::hardware_concurrency() / 2);
      if (plan.work_size > work_data.size())
        work_data.resize(plan.work_size);
      plan.work_data = work_data.data();

      auto status = ggml_graph_compute(graph, &plan);
      if (status != GGML_STATUS_SUCCESS)
        throw std::runtime_error("Failed to compute wk_b");

      auto name = std::string{"blk."} + std::to_string(il) + ".attn_k_b.weight";

      l.computed_wk_b = std::make_unique<ggml_tensor>(*wk_b);
      l.computed_wk_b->buffer = ggml_backend_buft_alloc_buffer(
          ggml_backend_buffer_get_type(l.wkv_b->buffer), ggml_nbytes(wk_b));
      l.computed_wk_b->data =
          ggml_backend_buffer_get_base(l.computed_wk_b->buffer);
      l.computed_wk_b->op =
          GGML_OP_NONE; // we absolutely need to do this, else the backend will
                        // attempt to find the parents of wk_b, which no longer
                        // exist, and will therefore crash.
      for (int j = 0; j < GGML_MAX_SRC; ++j)
        l.computed_wk_b->src[j] = nullptr;
      ggml_set_name(l.computed_wk_b.get(), name.c_str());
      ggml_backend_buffer_set_usage(l.computed_wk_b->buffer,
                                    GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
      ggml_backend_tensor_set(l.computed_wk_b.get(), wk_b->data, 0,
                              ggml_nbytes(wk_b));
      ...
      auto wv_b =
          ggml_cont(ctx, ggml_view_3d(ctx, &wkv_b, kv_lora_rank, n_embd_head_v,
                                      n_head, l.wkv_b->nb[1],
                                      l.wkv_b->nb[1] *
                                          (n_embd_head_qk_nope + n_embd_head_v),
                                      l.wkv_b->nb[1] * n_embd_head_qk_nope));
      ...
      if (status != GGML_STATUS_SUCCESS)
        throw std::runtime_error("Failed to compute wv_b");

      name = std::string{"blk."} + std::to_string(il) + ".attn_v_b.weight";

      l.computed_wv_b = std::make_unique<ggml_tensor>(*wv_b);
      l.computed_wv_b->buffer = ggml_backend_buft_alloc_buffer(
          ggml_backend_buffer_get_type(l.wkv_b->buffer), ggml_nbytes(wv_b));
      l.computed_wv_b->data =
          ggml_backend_buffer_get_base(l.computed_wv_b->buffer);
      l.computed_wv_b->op =
          GGML_OP_NONE; // we absolutely need to do this, else the backend will
                        // attempt to find the parents of wk_b, which no longer
                        // exist, and will therefore crash.
      for (int j = 0; j < GGML_MAX_SRC; ++j)
        l.computed_wv_b->src[j] = nullptr;
      ggml_set_name(l.computed_wv_b.get(), name.c_str());
      ggml_backend_buffer_set_usage(l.computed_wv_b->buffer,
                                    GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
      ggml_backend_tensor_set(l.computed_wv_b.get(), wv_b->data, 0,
                              ggml_nbytes(wv_b));
  ...
  //
  // Prepare wkv_b tensors to enable MLA=2,3 usage also for model files that
  // have been crippled to the mainline llama.cpp MLA implementation (MLA=1
  // here). We do it here because otherwise wk_b and wv_b may get
  // run-time-repacked, which will make preparation of wkv_b impossible. It also
  // has the benefit that wkv_b will get automatically run-time repacked if the
  // rtr option is set.
  //
  const int32_t n_head = hparams.n_head(0);
  std::vector<uint8_t> work_data;
  LLAMA_LOG_INFO("============ %s: need to compute %d wkv_b tensors\n",
                 __func__, n_to_compute);
  for (int il = 1; il < n_layer; ++il) {
    // Somehow the number of heads is being defined as being per layer. Not sure
    // why this is the case, but for now we do not support strange models that
    // have different numbers of heads in different model layers.
    if ((int)hparams.n_head(il) != n_head)
      throw std::runtime_error("Unsupported configuration");
  }
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    if (!ggml_backend_buffer_is_host(l.wv_b->buffer)) {
      ...
      if (wv_buffer.size() < nbytes)
        wv_buffer.resize(nbytes);
    ...
    tot_size +=
        n_wk * sizeof(float); // ggml_cont(ctx, ggml_transpose(ctx, wk_b_used));
    if (wv_b.type != GGML_TYPE_F32) {
      tot_size += n_wv * sizeof(float);
    }
    tot_size +=
        (n_wk + n_wv) *
        sizeof(float); // ggml_concat(ctx, wk_b_transposed, wv_b_used, 0);
    tot_size +=
        (n_wk + n_wv) * sizeof(float); // ggml_cast(ctx, wkv_b_f32, new_type);

    if (tmp_buffer.size() < tot_size)
      tmp_buffer.resize(tot_size);
    ...
    auto wkv_b_f32 = ggml_view_2d(ctx, wkv_b_f32_3d, wkv_b_f32_3d->ne[0],
                                  wkv_b_f32_3d->ne[1] * wkv_b_f32_3d->ne[2],
                                  wkv_b_f32_3d->nb[1], 0);

    auto new_type = wk_b.type == GGML_TYPE_BF16 && wv_b.type == GGML_TYPE_BF16
                        ? GGML_TYPE_BF16
                    : wk_b.type == GGML_TYPE_F16 && wv_b.type == GGML_TYPE_F16
                        ? GGML_TYPE_F16
                        : GGML_TYPE_Q8_0;
    ...
    auto status = ggml_graph_compute(graph, &plan);
    if (status != GGML_STATUS_SUCCESS)
      throw std::runtime_error("Failed to compute wkv_b");

    auto name = std::string{"blk."} + std::to_string(il) + ".attn_kv_b.weight";

    l.computed_wkv_b = std::make_unique<ggml_tensor>(*wkv_b);
    l.computed_wkv_b->buffer = ggml_backend_buft_alloc_buffer(
        ggml_backend_buffer_get_type(l.wk_b->buffer), ggml_nbytes(wkv_b));
    l.computed_wkv_b->data =
        ggml_backend_buffer_get_base(l.computed_wkv_b->buffer);
    l.computed_wkv_b->op =
        GGML_OP_NONE; // we absolutely need to do this, else the backend will
                      // attempt to find the parents of wkv_b, which no longer
                      // exist, and will therefore crash.
    for (int j = 0; j < GGML_MAX_SRC; ++j)
      l.computed_wkv_b->src[j] = nullptr;
    ggml_set_name(l.computed_wkv_b.get(), name.c_str());
    ggml_backend_buffer_set_usage(l.computed_wkv_b->buffer,
                                  GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
    ggml_backend_tensor_set(l.computed_wkv_b.get(), wkv_b->data, 0,
                            ggml_nbytes(wkv_b));
...
static bool item_in_list(const std::vector<std::string> &devices,
                         const char *name) {
  for (auto &device : devices) {
...
static void ggml_backend_add_from_device(llama_context *ctx,
...
static bool is_model_split_supported(const llama_model &model) {
  static std::unordered_set<llm_arch> k_supported = {
      LLM_ARCH_LLAMA,      LLM_ARCH_QWEN3MOE,     LLM_ARCH_GLM4_MOE,
      LLM_ARCH_MISTRAL3,   LLM_ARCH_COHERE2,      LLM_ARCH_MIMO2,
      LLM_ARCH_QWEN3,      LLM_ARCH_QWEN3VL,      LLM_ARCH_HUNYUAN_MOE,
      LLM_ARCH_OPENAI_MOE, LLM_ARCH_ERNIE4_5_MOE, LLM_ARCH_MINIMAX_M2,
      LLM_ARCH_SEED_OSS,   LLM_ARCH_STEP35,
  };
...
static bool llm_load_tensors(llama_model_loader &ml, llama_model &model,
  ...
  if (split_mode == LLAMA_SPLIT_MODE_GRAPH ||
    ...
    if (!is_model_split_supported(model)) {
      ...
      if (model.arch == LLM_ARCH_MIMO2 && model.devices.size() > 4 &&
        ...
        LLAMA_LOG_WARN("\n====================================================="
                       "===========\n");
        LLAMA_LOG_WARN("Split mode 'graph' for Mimo2 does not work with more "
                       "than 4 GPUs\n");
  ...
  if (int device_count = model.devices.size(); device_count > 1) {
    bool all_zero = tensor_split == nullptr ||
                    std::all_of(tensor_split, tensor_split + device_count,
  ...
  if (split_mode == LLAMA_SPLIT_MODE_LAYER) {
    ...
    for (int i = i_gpu_start; i < n_layer; ++i) {
      int layer_gpu =
          std::upper_bound(model.splits.begin(),
                           ...
                           float(i - i_gpu_start) / act_gpu_layers) -
          model.splits.begin();
      model.buft_layer[i] =
    ...
    if (n_gpu_layers > n_layer) {
      int layer_gpu =
          std::upper_bound(model.splits.begin(),
                           model.splits.begin() + device_count,
                           float(act_gpu_layers - 1) / act_gpu_layers) -
          model.splits.begin();
      model.buft_output =
          llama_default_buffer_type_offload(model, model.devices[layer_gpu]);
    } else {
    ...
    if ((split_mode == LLAMA_SPLIT_MODE_GRAPH ||
      ...
      // LLAMA_SPLIT_MODE_NONE or LLAMA_SPLIT_MODE_LAYER in backends where it is
      // not supported
      split_buft =
    ...
    // assign the output layer
    if (n_gpu_layers > n_layer) {
      model.buft_output = {split_buft, llama_default_buffer_type_offload(
                                           model, model.devices[main_gpu])};
    } else {
      model.buft_output = llama_default_buffer_type_cpu(true);
    }
  }
  ...
  LLAMA_LOG_INFO("%s: ggml ctx size = %7.2f MiB\n", __func__,
                 model.ctxs.size() * ctx_size / 1024.0 / 1024.0);
  ...
  // Ensure we have enough capacity for the maximum backend buffer we will
  // potentially create
  ...
  for (auto &it : ctx_map) {
    ...
    // only the mmap region containing the tensors in the model is mapped to the
    // backend buffer this is important for metal with apple silicon: if the
    // entire model could be mapped to a metal buffer, then we could just use
    // metal for all layers this allows using partial offloading when the model
    // size exceeds the metal buffer size, but not the RAM size
...
#ifdef GGML_USE_METAL
    else if (ml.use_mmap && use_mmap_buffer &&
             buft == ggml_backend_metal_buffer_type()) {
      for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
        ...
        ggml_backend_buffer_t buf = ggml_backend_metal_buffer_from_ptr(
            (char *)addr + first, last - first, max_size);
        if (buf == nullptr) {
    ...
    for (auto &buf : bufs) {
      ...
      // this is used by ggml_backend_sched to improve op scheduling -> ops that
      // use a weight are preferably scheduled to the backend that contains the
      // weight
  ...
  for (ggml_context *ctx : model.ctxs) {
    for (auto *cur = ggml_get_first_tensor(ctx); cur != NULL;
  ...
  for (auto &it : ctx_bufs) {
    ...
    auto &bufs = it.second;
    if (!ml.load_all_data(ctx, bufs, use_mlock ? &model.mlock_mmaps : NULL,
  ...
  if (!ml.use_mmap) {
    ...
    for (auto &it : model.tensors_by_name) {
      if (ggml_backend_buffer_is_host(it.second->buffer)) {
        if (iqk_modify_tensor(it.second))
          ++n_modified;
      }
    }
    if (n_modified > 0)
      printf("============ Modified %d tensors\n", n_modified);
  }
  ...
  if (validate_quants) {
    ...
    for (auto &it : model.tensors_by_name) {
      if (ggml_backend_buffer_is_host(it.second->buffer)) {
        if (!iqk_validate_tensor(it.second))
          ++nbad;
      }
    }
    if (nbad > 0) {
  ...
  if (!ml.use_mmap && ml.repack_tensors) {
    ...
    for (auto &it : model.tensors_by_name) {
      if (ggml_backend_buffer_is_host(it.second->buffer)) {
        ...
        if (it.second->view_src)
          continue;
        iqk_repack_tensor(it.second);
        if (it.second->type != orig_type)
          ++n_repacked;
      }
    }
    if (n_repacked > 0)
      printf("============ Repacked %d tensors\n", n_repacked);
  }
...
// Returns 0 on success, -1 on error, and -2 on cancellation via
...
static int llama_model_load(const std::string &fname, llama_model &model,
  ...
  try {
    llama_model_loader ml(fname, params.use_mmap, params.check_tensors,
                          params.repack_tensors, params.use_thp,
                          params.merge_qkv, params.merge_up_gate_exps,
    ...
    if (params.n_gpu_layers > 0 &&
      ...
      // TODO(cebtenzzre): propagate this error outside of
      // llama_load_model_from_file
      LLAMA_LOG_WARN("%s: disabling Kompute due to unsupported model arch or "
                     "quantization\n",
                     __func__);
...
static int32_t llama_relative_position_bucket(llama_pos x, llama_pos y,
                                              uint64_t n_buckets,
                                              bool bidirectional) {
  ...
  int32_t relative_position_if_large =
      floorf(max_exact + logf(1.0 * relative_position / max_exact) *
                             (n_buckets - max_exact) /
                             log(1.0 * max_distance / max_exact));
  relative_position_if_large =
      std::min<int32_t>(relative_position_if_large, n_buckets - 1);
  relative_bucket +=
      (relative_position < max_exact ? relative_position
                                     : relative_position_if_large);
...
static void llama_set_inputs(llama_context &lctx, const llama_batch &batch) {
  ...
  if (batch.token) {
    ...
    ggml_backend_tensor_set(lctx.inp_tokens, batch.token, 0,
                            n_tokens * ggml_element_size(lctx.inp_tokens));
#if IK_PRINT_TIMING == 2
  ...
  if (batch.embd) {
    ...
    ggml_backend_tensor_set(lctx.inp_embd, batch.embd, 0,
                            n_tokens * n_embd *
                                ggml_element_size(lctx.inp_embd));
#if IK_PRINT_TIMING == 2
    auto tim2 = ggml_time_us();
  ...
  if (batch.pos && lctx.inp_pos) {
    ...
    if (batch.token && n_pos_per_embd == 4) {
      ...
      ggml_backend_tensor_set(lctx.inp_pos, pos_data.data(), 0,
                              pos_data.size() *
                                  ggml_element_size(lctx.inp_pos));
    } else {
      ggml_backend_tensor_set(lctx.inp_pos, batch.pos, 0,
  ...
  if (lctx.inp_pos && lctx.inp_scale) {
    ...
    if (int(lctx.scale_data.size()) < n_tokens)
      lctx.scale_data.resize(n_tokens);
    int n_pos_per_token = 1;
    for (int i = 0; i < n_tokens; ++i) {
      lctx.scale_data[i] =
          std::log(std::floor((batch.pos[i] + 1.0f) /
                              hparams.n_attn_temp_floor_scale) +
                   1.0f) *
              hparams.f_attn_temp_scale +
          1.0f;
    }
    ggml_backend_tensor_set(lctx.inp_scale, lctx.scale_data.data(), 0,
                            n_tokens * n_pos_per_token *
                                ggml_element_size(lctx.inp_scale));
#if IK_PRINT_TIMING == 2
    auto tim2 = ggml_time_us();
  ...
  if (hparams.causal_attn || cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
    ...
    const int64_t n_tokens = batch.n_tokens;
    if (n_tokens > 1) {
      GGML_ASSERT(lctx.inp_out_ids &&
                  "every model that can must skip unused outputs");
    }
  ...
  if (lctx.inp_KQ_mask || lctx.inp_KQ_mask_swa) {
    ...
    auto tim1 = ggml_time_us();
#endif
    // NOTE: hparams.causal_attn indicates the model is capable of generation
    // and uses the kv cache.
    if (cparams.causal_attn && !lctx.is_encoding) {
      ...
      auto noalibi_f16 = [&lctx, &hparams, n_kv, data_f16, data_swa_f16](
                             int j, llama_pos pos, llama_seq_id seq_id,
                             int first, int last) {
        ggml_half h_inf = ggml_fp32_to_fp16(-INFINITY);
        ggml_half h_zero = ggml_fp32_to_fp16(0.f);
        for (int i = first; i < last; ++i) {
          ggml_half h = !lctx.kv_self.cells[i].has_seq_id(seq_id) ||
            ...
            if (h != h_inf) {
              if (hparams.n_attn_chunk) {
                llama_pos pos_chunk_start =
                    (pos / hparams.n_attn_chunk) * hparams.n_attn_chunk;
                if (lctx.kv_self.cells[i].pos < pos_chunk_start ||
                    pos < pos_chunk_start) {
                  h = h_inf;
                }
              } else {
      ...
      if (n_kv >= 1024 && n_tokens >= 32) {
        ...
        auto compute = [&batch, &lctx, &hparams, &cparams, &noalibi_f16,
                        n_tokens, n_kv, npt, data, data_swa, data_f16,
                        data_swa_f16](int ith) {
          int first = ith * npt;
              ...
              if (data_swa || data_swa_f16) {
                if (f > -INFINITY) {
                  if (hparams.n_attn_chunk) {
                    ...
                    if (pos - lctx.kv_self.cells[i].pos >=
                        (int32_t)hparams.n_swa) {
                      f = -INFINITY;
                    }
                  }
                }
        ...
        if (n_tokens_padded > n_tokens) {
          if (data) {
            std::fill(data + int64_t(n_tokens) * n_kv,
                      data + n_tokens_padded * n_kv, -INFINITY);
          }
          if (data_f16) {
            ggml_half h_inf = ggml_fp32_to_fp16(-INFINITY);
            std::fill(data_f16 + int64_t(n_tokens) * n_kv,
                      data_f16 + n_tokens_padded * n_kv, h_inf);
          }
          if (data_swa) {
            std::fill(data_swa + int64_t(n_tokens) * n_kv,
                      data_swa + n_tokens_padded * n_kv, -INFINITY);
          }
          if (data_swa_f16) {
            ggml_half h_inf = ggml_fp32_to_fp16(-INFINITY);
            std::fill(data_swa_f16 + int64_t(n_tokens) * n_kv,
                      data_swa_f16 + n_tokens_padded * n_kv, h_inf);
          }
        }
      } else {
        ...
        // It's assumed that if a token in the batch has multiple sequences,
        // they are equivalent.
        for (int h = 0; h < 1; ++h) {
          for (int j = 0; j < n_tokens; ++j) {
            const llama_pos pos = batch.pos[j];
            ...
            for (int i = 0; i < n_kv; ++i) {
              ...
              if (data_f16) {
                data_f16[h * (n_kv * n_tokens) + j * n_kv + i] =
                    ggml_fp32_to_fp16(f);
              }
              ...
              if (data_swa || data_swa_f16) {
                ...
                if (data_swa_f16) {
                  data_swa_f16[h * (n_kv * n_tokens) + j * n_kv + i] =
                      ggml_fp32_to_fp16(f);
                }
              }
            }
      ...
      const int64_t n_tokens = batch.n_tokens;
      const int64_t n_stride =
          hparams.causal_attn && !lctx.is_encoding ? kv_self.n : n_tokens;
  ...
  if (cparams.embeddings && cparams.pooling_type == LLAMA_POOLING_TYPE_MEAN) {
    ...
    memset(lctx.inp_mean->data, 0,
           n_tokens * n_tokens * ggml_element_size(lctx.inp_mean));
    ...
    for (int i = 0; i < n_tokens; ++i) {
      ...
      GGML_ASSERT(
          seq_id < n_tokens &&
          "seq_id cannot be larger than n_tokens with pooling_type == MEAN");
  ...
  if (cparams.embeddings && cparams.pooling_type == LLAMA_POOLING_TYPE_CLS) {
    ...
    for (int i = 0; i < n_tokens; ++i) {
      ...
      GGML_ASSERT(
          seq_id < n_tokens &&
          "seq_id cannot be larger than n_tokens with pooling_type == CLS");
  ...
  if (cparams.embeddings && cparams.pooling_type == LLAMA_POOLING_TYPE_LAST) {
    ...
    for (int i = 0; i < n_tokens; ++i) {
      ...
      GGML_ASSERT(
          seq_id < n_tokens &&
          "seq_id cannot be larger than n_tokens with pooling_type == LAST");
  ...
  if (kv_self.recurrent) {
    ...
    // they are assumed to be equivalent (not here, but in ggml_ssm_scan and
    // ggml_ssm_conv).
    if (lctx.inp_s_seq) {
      const int64_t n_tokens = batch.n_tokens;
      ...
      for (int j = 0; j < n_tokens; ++j) {
        ...
        for (int i = 0; i < n_kv; ++i) {
          if (i < n_seq) {
            // for this type of model, the head is the minimum seq_id of the
            // batch
          ...
          for (int i = 0; i < n_kv; ++i) {
            data[h * (n_kv * n_tokens) + j * n_kv + i] =
                llama_relative_position_bucket(
                    lctx.kv_self.cells[i].pos, batch.pos[j],
                    hparams.n_rel_attn_bkts, lctx.is_encoding);
          }
        }
      }
    } else {
        ...
        for (int j = 0; j < n_tokens; ++j) {
          for (int i = 0; i < n_tokens; ++i) {
            data[h * (n_tokens * n_tokens) + j * n_tokens + i] =
                llama_relative_position_bucket(batch.pos[i], batch.pos[j],
  ...
  if (!lctx.is_encoding && lctx.inp_embd_enc) {
    ...
    ggml_backend_tensor_set(lctx.inp_embd_enc, lctx.embd_enc.data(), 0,
                            ggml_nbytes(lctx.inp_embd_enc));
  }
    ...
    for (int h = 0; h < 1; ++h) {
      ...
      for (int i = n_tokens; i < GGML_PAD(n_tokens, GGML_KQ_MASK_PAD); ++i) {
        for (int j = 0; j < n_output_enc; ++j) {
          data[h * (n_output_enc * n_tokens) + i * n_output_enc + j] =
              -INFINITY;
        }
      }
    }
...
static size_t llama_output_reserve(llama_context &lctx, size_t n_outputs) {
  ...
  const size_t prev_size =
      lctx.buf_output ? ggml_backend_buffer_get_size(lctx.buf_output) : 0;
  ...
  if (!lctx.buf_output || prev_size < new_size) {
    if (lctx.buf_output) {
#ifndef NDEBUG
      // This doesn't happen often, but may be annoying in some cases (like the
      // HellaSwag benchmark)
      LLAMA_LOG_INFO(
    ...
    lctx.buf_output = ggml_backend_buft_alloc_buffer(
        llama_default_buffer_type_cpu(true), new_size);
...
static void llama_graph_compute(llama_context &lctx, ggml_cgraph *gf,
                                int n_threads) {
#ifdef GGML_USE_METAL
  ...
  if (lctx.backend_cpu != nullptr) {
    ...
    ggml_backend_cpu_set_abort_callback(lctx.backend_cpu, lctx.abort_callback,
                                        lctx.abort_callback_data);
  }
#ifdef GGML_USE_BLAS
...
//
static int
llama_decode_internal(llama_context &lctx,
  ...
  GGML_ASSERT((!batch_all.token && batch_all.embd) ||
              (batch_all.token && !batch_all.embd)); // NOLINT
  ...
  // this indicates we are doing pooled embedding, so we ignore batch.logits and
  // output all tokens
  const bool embd_pooled =
      cparams.embeddings && cparams.pooling_type != LLAMA_POOLING_TYPE_NONE;
  ...
  for (uint32_t cur_token = 0; cur_token < n_tokens_all;
    ...
    llama_batch u_batch = {
        ...
        /* .token      = */ batch_all.token ? batch_all.token + cur_token
                                            : nullptr,
        /* .embd       = */ batch_all.embd ? batch_all.embd + cur_token * n_embd
                                           : nullptr,
        /* .pos        = */ batch_all.pos ? batch_all.pos + cur_token : nullptr,
        /* .n_seq_id   = */ batch_all.n_seq_id ? batch_all.n_seq_id + cur_token
                                               : nullptr,
        /* .seq_id     = */ batch_all.seq_id ? batch_all.seq_id + cur_token
                                             : nullptr,
        /* .logits     = */ batch_all.logits ? batch_all.logits + cur_token
                                             : nullptr,
        /* .all_pos_0  = */ batch_all.all_pos_0 +
            (llama_pos)cur_token * batch_all.all_pos_1,
    ...
    if (hparams.causal_attn) {
      ...
      //   better to start searching from the beginning of the cache, hoping to
      //   fill it
      ...
      if (!kv_self.recurrent) {
        ...
        // utilized after enough generations, the benefit from this heuristic
        // disappears if we start defragmenting the cache, the benefit from this
        // will be more important
        const uint32_t pad = llama_kv_cache_get_padding(cparams);
        kv_self.n = std::min(
            kv_self.size,
            std::max(pad, GGML_PAD(llama_kv_cache_cell_max(kv_self), pad)));
    ...
    //     *)batch_all.embd, (const void *)batch_all.logits, (const void
    //     *)batch_all.token); printf("    n_outputs = %d, kv_self.n = %d\n",
    //     n_outputs, kv_self.n);
    // }
    ...
    if (!lctx.can_reuse_graph(u_batch)) {
      ...
      ggml_backend_sched_set_eval_callback(lctx.sched, lctx.cparams.cb_eval,
...
#endif
      if (u_batch.n_tokens == 1 && u_batch.embd == nullptr &&
          lctx.cparams.graph_reuse) {
        lctx.prev = std::make_unique<llama_context::Prev>(
            llama_context::Prev{(int)u_batch.all_seq_id, (int)lctx.n_outputs,
                                (int)lctx.kv_self.n, gf});
      }
    } else {
    ...
    if (res) {
      ...
      ggml_backend_t backend_res =
          ggml_backend_sched_get_tensor_backend(lctx.sched, res);
      ...
      if (n_outputs_new) {
        ...
        GGML_ASSERT((n_outputs_prev + n_outputs_new) * n_vocab <=
                    (int64_t)lctx.logits_size);
    ...
    if (embd) {
      ...
      ggml_backend_t backend_embd =
          ggml_backend_sched_get_tensor_backend(lctx.sched, embd);
      ...
      case LLAMA_POOLING_TYPE_NONE: {
        ...
        if (n_outputs_new) {
          GGML_ASSERT(n_outputs_prev + n_outputs_new <= n_outputs);
          GGML_ASSERT((n_outputs_prev + n_outputs_new) * n_embd <=
                      (int64_t)lctx.embd_size);
      ...
      case LLAMA_POOLING_TYPE_LAST: {
        ...
        for (uint32_t i = 0; i < n_tokens; i++) {
          ...
          ggml_backend_tensor_get_async(
              backend_embd, embd, embd_seq_out[seq_id].data(),
  ...
  // set to total number of outputs in the batch, for use in
  // llama_get_logits_ith
  lctx.n_outputs = n_outputs;

  // wait for the computation to finish (automatically done when obtaining the
  // model output)
  // llama_synchronize(&lctx);
  ...
  if (cparams.causal_attn && cparams.defrag_thold >= 0.0f) {
    const float fragmentation =
        kv_self.n >= 128 ? 1.0f - float(kv_self.used) / float(kv_self.n) : 0.0f;
...
// return negative int on error
//
static int llama_encode_internal(llama_context &lctx, llama_batch batch) {
  ...
  // micro-batching is not possible for non-causal encoding, so we process the
  // batch in a single shot
  ...
  const int n_threads =
      n_tokens == 1 ? cparams.n_threads : cparams.n_threads_batch;
  GGML_ASSERT(n_threads > 0);
  ...
  if (llama_model_has_decoder(&lctx.model)) {
    // first case is an encoder-decoder T5 model where embeddings are passed to
    // decoder
  ...
  if (embd) {
    ...
    if (llama_model_has_decoder(&lctx.model)) {
      ...
      // remember the sequence ids used during the encoding - needed for cross
      // attention later
...
// find holes from the beginning of the KV cache and fill them by moving data
...
static void llama_kv_cache_defrag_internal(struct llama_context &lctx) {
  ...
  // TODO: tmp fix
  // https://github.com/ggerganov/llama.cpp/issues/6685#issuecomment-2057579516
  const uint32_t max_moves =
      (lctx.model.max_nodes() - 2 * n_layer) / (6 * n_layer);
...
static int32_t llama_kv_cache_update_internal(struct llama_context &lctx) {
  ...
  if (need_reserve) {
    ...
    llama_token token = llama_token_bos(
        &lctx.model); // not actually used by llama_build_graph, but required to
                      // choose between token and embedding inputs graph
    ggml_cgraph *gf = llm_build_context::llama_build_graph(
        lctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true);
...
static void
llama_lora_adapter_init_internal(struct llama_model *model,
  ...
  struct gguf_context *ctx_gguf =
      gguf_init_from_file(path_lora, meta_gguf_params);
  if (!ctx_gguf) {
  ...
  // check metadata
  {
    ...
    if (adapter_type != "lora") {
      gguf_free(ctx_gguf);
      throw std::runtime_error("expect adapter.type to be 'lora', but got: " +
                               adapter_type);
    }
  ...
  for (auto &it : ab_map) {
    ...
    struct ggml_context *dev_ctx =
        get_ctx_for_buft(ggml_backend_buffer_get_type(model_tensor->buffer));
    // validate tensor shape
    if (model_tensor->ne[0] != w.a->ne[0] ||
    ...
    if (w.a->ne[1] != w.b->ne[0]) {
      ...
      ggml_free(ctx);
      throw std::runtime_error(
          "lora_a tensor is not transposed (hint: adapter from \"finetune\" "
          "example is no longer supported)");
    }
  ...
  // allocate tensors / buffers and zero
  {
    ...
    for (auto it : ctx_map) {
      ...
      ggml_backend_buffer_t buf =
          ggml_backend_alloc_ctx_tensors_from_buft(ctx_dev, buft);
      if (!buf) {
        gguf_free(ctx_gguf);
        ggml_free(ctx);
        throw std::runtime_error(
  ...
  // set tensor data
  {
    ...
    auto set_tensor = [&](struct ggml_tensor *orig, struct ggml_tensor *dev) {
      size_t offs = gguf_get_data_offset(ctx_gguf) +
                    gguf_get_tensor_offset(
                        ctx_gguf, gguf_find_tensor(ctx_gguf, orig->name));
...
int32_t llama_lora_adapter_set(struct llama_context *ctx,
                               struct llama_lora_adapter *adapter,
                               float scale) {
...
int32_t llama_lora_adapter_remove(struct llama_context *ctx,
                                  struct llama_lora_adapter *adapter) {
...
#ifdef GGML_USE_METAL
  // note: we usually have plenty of VRAM, so by default offload all layers to
  // the GPU
...
bool llama_supports_mmap(void) { return llama_mmap::SUPPORTED; }

bool llama_supports_mlock(void) { return llama_mlock::SUPPORTED; }

bool llama_supports_gpu_offload(void) {
#if defined(GGML_USE_CUDA) || defined(GGML_USE_METAL) ||                       \
    defined(GGML_USE_VULKAN) || defined(GGML_USE_SYCL) ||                      \
...
llama_load_model_from_file(const char *path_model,
  ...
  // if device is specified, only those in the devices are included in the
  // model->devices
  ...
  for (idx = 0; idx < dev_count; idx++) {
    ggml_backend_buffer_type_t buft =
        llama_default_buffer_type_offload(*model, idx);
  ...
  if (has_rpc) {
    model->rpc_servers =
        extract_device_from_rpc_device(string_split(params.rpc_servers, ","));
  ...
  if (!model->devices.size()) {
    ...
    for (auto i : model->devices) {
      ggml_backend_buffer_type_t buft =
          llama_default_buffer_type_offload(*model, i);
      const char *name = ggml_backend_buft_name(buft);
...
static void llama_repack_up_gate_exps(llama_context &lctx) {
  ...
  for (int il = 0; il < int(model.layers.size()); ++il) {
    auto &l = model.layers[il];
    if (l.ffn_up_gate_exps && l.ffn_up_exps && l.ffn_gate_exps) {
      GGML_ASSERT(l.ffn_up_gate_exps->type == l.ffn_up_exps->type &&
                  l.ffn_up_gate_exps->type == l.ffn_gate_exps->type);
      GGML_ASSERT(l.ffn_up_gate_exps->ne[0] == l.ffn_up_exps->ne[0] &&
                  l.ffn_up_gate_exps->ne[0] == l.ffn_gate_exps->ne[0]);
      GGML_ASSERT(l.ffn_up_gate_exps->ne[2] == l.ffn_up_exps->ne[2] &&
                  l.ffn_up_gate_exps->ne[2] == l.ffn_gate_exps->ne[2]);
      GGML_ASSERT(l.ffn_up_gate_exps->ne[1] ==
                  l.ffn_up_exps->ne[1] + l.ffn_gate_exps->ne[1]);
      ...
      ggml_backend_tensor_get(l.ffn_gate_exps, aux_buffer_gate.data(), 0,
                              nbytes);
      ...
      for (int i2 = 0; i2 < (int)l.ffn_up_gate_exps->ne[2]; ++i2) {
        std::memcpy(aux_buffer_up_gate.data() + offset_up_gate,
                    aux_buffer_up.data() + offset_up, expert_size);
        offset_up_gate += expert_size;
        std::memcpy(aux_buffer_up_gate.data() + offset_up_gate,
                    aux_buffer_gate.data() + offset_up, expert_size);
        offset_up_gate += expert_size;
        offset_up += expert_size;
      }
      ggml_backend_tensor_set(l.ffn_up_gate_exps, aux_buffer_up_gate.data(), 0,
                              2 * expert_size * l.ffn_up_gate_exps->ne[2]);
      if (l.ffn_up_gate_exps_b && l.ffn_up_exps_b && l.ffn_gate_exps_b) {
        ...
        printf("%s: repacking up/gate experts bias in layer %d\n", __func__,
               il);
        ggml_backend_tensor_get(l.ffn_up_exps_b, aux_buffer_up.data(), 0,
                                nbytes);
        ggml_backend_tensor_get(l.ffn_gate_exps_b, aux_buffer_gate.data(), 0,
                                nbytes);
        if (aux_buffer_up_gate.size() < 2 * nbytes) {
          aux_buffer_up_gate.resize(2 * nbytes);
        }
        ...
        ggml_backend_tensor_set(l.ffn_up_gate_exps_b, aux_buffer_up_gate.data(),
                                0,
                                2 * expert_size * l.ffn_up_gate_exps_b->ne[1]);
      }
    }
  }
}
...
llama_new_context_with_model(struct llama_model *model,
  ...
  printf("===================================== %s: %s\n", __func__,
         ggml_type_name(params.type_reduce));
  ...
  // if (params.flash_attn && model->hparams.n_embd_head_k !=
  // model->hparams.n_embd_head_v) {
  //     LLAMA_LOG_WARN("%s: flash_attn requires n_embd_head_k == n_embd_head_v
  //     - forcing off\n", __func__); params.flash_attn = false;
  // }

  if (params.type_v != GGML_TYPE_F16 && params.type_v != GGML_TYPE_BF16 &&
      !params.flash_attn) {
  ...
  if (params.k_cache_hadamard && !ggml_is_quantized(params.type_k)) {
    LLAMA_LOG_WARN("%s: there is no point in Hadamard transforms with not "
                   "quantized K-cache. Turning Hadamard off\n",
                   __func__);
  ...
  cparams.n_batch = hparams.causal_attn
                        ? std::min(cparams.n_ctx, params.n_batch)
                        : params.n_batch;

  // the batch has to be at least GGML_KQ_MASK_PAD because we will be padding
  // the KQ_mask this is required by GPU kernels in order to avoid out-of-bounds
  // accesses (e.g. ggml_flash_attn_ext) ref:
  ...
  cparams.n_ubatch = std::min(
      cparams.n_batch, params.n_ubatch == 0 ? params.n_batch : params.n_ubatch);
  ...
  if (cparams.yarn_ext_factor < 0.0f) { // negative indicates 'not set'
    cparams.yarn_ext_factor =
        rope_scaling_type == LLAMA_ROPE_SCALING_TYPE_YARN ? 1.0f : 0.0f;
  }
  ...
  if (model->arch != LLM_ARCH_DEEPSEEK2 && cparams.mla_attn != 0) {
    // LLAMA_LOG_WARN("=====================================================================\n");
    // LLAMA_LOG_WARN(" MLA is only available for LLM_ARCH_DEEPSEEK2 -> turning
    // off MLA\n");
    // LLAMA_LOG_WARN("=====================================================================\n");
    cparams.mla_attn = 0;
  }
  if (model->arch == LLM_ARCH_OPENAI_MOE &&
      model->split_mode == LLAMA_SPLIT_MODE_GRAPH) {
    // if (cparams.split_mode_f16) {
    //     LLAMA_LOG_WARN("=====================================================================\n");
    //     LLAMA_LOG_WARN("GPT-OSS with split mode graph requires f32
    //     precision\n"); LLAMA_LOG_WARN("    => changing cparams.split_mode_f16
    //     to 'false'\n");
    //     LLAMA_LOG_WARN("=====================================================================\n");
  ...
  LLAMA_LOG_INFO("%s: fused_moe     = %d\n", __func__,
                 cparams.fused_moe_up_gate);
  LLAMA_LOG_INFO("%s: grouped er    = %d\n", __func__,
                 cparams.grouped_expert_routing);
  ...
  if (model->arch == LLM_ARCH_MAMBA) {
    // Mamba needs at least as many KV cells as there are sequences kept at any
    // time
  ...
  if (!hparams.vocab_only) {
    ...
    if (model->split_mode == LLAMA_SPLIT_MODE_NONE) {
      // with split_mode LLAMA_SPLIT_MODE_NONE or LLAMA_SPLIT_MODE_GRAPH, only
      // the main GPU backend is used
      ggml_backend_t backend =
          ggml_backend_cuda_init(model->main_gpu, cparams.cuda_params);
      if (backend == nullptr) {
      ...
      // LLAMA_SPLIT_MODE_LAYER and LLAMA_SPLIT_MODE_GRAPH require a backend for
      // each GPU
      ...
      if (false && model->split_mode == LLAMA_SPLIT_MODE_GRAPH) {
        ...
        if (params)
          new_params = std::string{(const char *)params} + ',';
        new_params += extra_string;
        params = new_params.data();
      }
      for (int device = 0; device < ggml_backend_cuda_get_device_count();
           ++device) {
    ...
    if (model->split_mode == LLAMA_SPLIT_MODE_GRAPH ||
        model->split_mode == LLAMA_SPLIT_MODE_ATTN) {
      LLAMA_LOG_ERROR("%s: split mode 'graph' or 'attn' not supported. Failed "
                      "to initialize Vulkan backend\n",
                      __func__);
      llama_free(ctx);
      return nullptr;
    }
    if (model->split_mode == LLAMA_SPLIT_MODE_NONE) {
      ...
      for (int device = 0; device < ggml_backend_vk_get_device_count();
           ++device) {
...
#elif defined(GGML_USE_SYCL)
    // with split_mode LLAMA_SPLIT_MODE_NONE or LLAMA_SPLIT_MODE_GRAPH, only the
    // main GPU backend is used
    if (model->split_mode == LLAMA_SPLIT_MODE_NONE ||
        model->split_mode == LLAMA_SPLIT_MODE_GRAPH) {
    ...
    // with split_mode LLAMA_SPLIT_MODE_NONE or LLAMA_SPLIT_MODE_GRAPH, only the
    // main GPU backend is used
    // TODO: ggml_backend_cann is not support split tensor now, just leave code
    // here.
    if (model->split_mode == LLAMA_SPLIT_MODE_NONE ||
        model->split_mode == LLAMA_SPLIT_MODE_GRAPH) {
      ...
      // TODO: currently, CANN can't use multi-gpus, just leave code here for
      // further cann version.
      for (int32_t device = 0; device < ggml_backend_cann_get_device_count();
           ++device) {
    ...
    if (model->n_gpu_layers > 0) {
      for (const auto &device : model->rpc_servers) {
        ggml_backend_t backend =
            ggml_backend_rpc_init(device.endpoint.c_str(), device.device);
        if (backend == nullptr) {
          LLAMA_LOG_ERROR("%s: failed to initialize RPC%d to '%s'\n", __func__,
                          device.device, device.endpoint.c_str());
          llama_free(ctx);
          return nullptr;
        }
        ggml_backend_add_from_device(ctx, backend);
    ...
    if (!llama_kv_cache_init(ctx->kv_self, ctx, type_k, type_v, kv_size,
                             cparams.offload_kqv)) {
      LLAMA_LOG_ERROR(
...

    {
      ...
      if (memory_size_k + memory_size_v > 0) {
        if (cparams.mla_attn != 0 && !cparams.flash_attn) {
          LLAMA_LOG_INFO("%s: KV self size  = %7.2f MiB, c^KV (%s): %7.2f MiB, "
                         "kv^T (%s): %7.2f MiB\n",
                         __func__,
                         (float)(memory_size_k + memory_size_v) /
                             (1024.0f * 1024.0f),
          ...
          LLAMA_LOG_INFO("%s: KV self size  = %7.2f MiB, K (%s): %7.2f MiB, V "
                         "(%s): %7.2f MiB\n",
                         __func__,
                         (float)(memory_size_k + memory_size_v) /
                             (1024.0f * 1024.0f),
    ...
    // graph outputs buffer
    {
      ...
      LLAMA_LOG_INFO("%s: %10s  output buffer size = %8.2f MiB\n", __func__,
                     ...
                     ggml_backend_buffer_get_size(ctx->buf_output) / 1024.0 /
                         1024.0);
    }
    ...
    // scheduler and compute buffers
    {
      ...
      // enabling pipeline parallelism in the scheduler increases memory usage,
      // so it is only done when necessary
      ...
      ggml_cgraph *gf = llm_build_context::llama_build_graph(
          *ctx, llama_batch_get_one(&token, n_tokens, n_past, 0), true);

      // initialize scheduler with the worst-case graph
      bool gf_success = ggml_backend_sched_reserve(ctx->sched, gf);
      if (!gf_success) {
        if (pipeline_parallel) {
          LLAMA_LOG_WARN("%s: compute buffer allocation failed, retrying "
                         "without pipeline parallelism\n",
                         __func__);
          ctx->sched =
              ggml_backend_sched_new(ctx->backends.data(), backend_buft.data(),
      ...
      // note: the number of splits during measure is higher than during
      // inference due to the kv shift
  ...
  if (params.offload_policy) {
    const std::vector<std::pair<int, int>> &policy =
        *(const std::vector<std::pair<int, int>> *)params.offload_policy;
    for (auto [op, on_off] : policy) {
      if (op < 0 || op >= int(GGML_OP_COUNT)) {
        LLAMA_LOG_INFO(
            "XXXXXXXXXXXXXXXXXXXXX Setting offload policy for all ops to %s\n",
            on_off ? "ON" : "OFF");
      } else {
        LLAMA_LOG_INFO(
            "XXXXXXXXXXXXXXXXXXXXX Setting offload policy for op %s to %s\n",
  ...
  if (model->split_mode == LLAMA_SPLIT_MODE_GRAPH &&
    ...
    if (model->has_tensor_overrides() && cparams.split_mode_graph_scheduling) {
      ...
      LLAMA_LOG_INFO(
          "XXXXXXXX It may or might NOT infer properly due to unsupported "
          "combinations between SMGS and every possible tensor overrides.\n");
    }
  }
...
const struct llama_vocab *
llama_get_model_vocab(const struct llama_model *model) {
  return &model->vocab;
}
...
enum llama_rope_type llama_rope_type(const struct llama_model *model) {
  ...
  // use what we call a normal RoPE, operating on pairs of consecutive head
  // values
  case LLM_ARCH_LLAMA:
  case LLM_ARCH_DECI:
...
int32_t llama_model_meta_val_str(const struct llama_model *model,
...
int32_t llama_model_meta_key_by_index(const struct llama_model *model, int i,
...
int32_t llama_model_meta_val_str_by_index(const struct llama_model *model,
                                          int32_t i, char *buf,
                                          size_t buf_size) {
  if (i < 0 || i >= (int)model->gguf_kv.size()) {
    if (buf_size > 0) {
...
int32_t llama_model_desc(const struct llama_model *model, char *buf,
                         size_t buf_size) {
  return snprintf(buf, buf_size, "%s %s %s", llama_model_arch_name(model->arch),
                  llama_model_type_name(model->type),
                  llama_model_ftype_name(model->ftype).c_str());
}
...
const char *llama_model_chat_template(const struct llama_model *model,
                                      const char *name) {
  const auto key =
      name ? LLM_KV(model->arch, name)(LLM_KV_TOKENIZER_CHAT_TEMPLATE)
  ...
  if (it == model->gguf_kv.end()) {
    ...
    if (!name && pre_type == LLAMA_VOCAB_PRE_TYPE_TEKKEN &&
        model->layers.size() == 40) {
...
struct ggml_tensor *llama_get_model_tensor(struct llama_model *model,
                                           const char *name) {
  auto it = std::find_if(
      model->tensors_by_name.begin(), model->tensors_by_name.end(),
...
struct llama_lora_adapter *llama_lora_adapter_init(struct llama_model *model,
                                                   const char *path_lora) {
  try {
...
static bool llama_control_vector_init(struct llama_control_vector &cvec,
  ...
  for (size_t il = 1; il < model.hparams.n_layer; il++) {
    ...
    ggml_tensor *tensor =
        ggml_new_tensor_1d(ctx, GGML_TYPE_F32, model.hparams.n_embd);
...
int32_t llama_control_vector_apply(struct llama_context *lctx,
  ...
  for (size_t il = 1; il < model.hparams.n_layer; il++) {
    ...
    const size_t off =
        n_embd *
        (il -
         1); // buffer doesn't have data for layer 0, since it's never present
    if (off + n_embd <= len) {
      ggml_backend_tensor_set(cvec.tensors[il], data + off, 0,
...
void llama_kv_cache_view_update(const struct llama_context *ctx,
                                struct llama_kv_cache_view *view) {
  if (uint32_t(view->n_cells) < ctx->kv_self.size || view->cells == nullptr) {
    view->n_cells = int32_t(ctx->kv_self.size);
    void *p = realloc(view->cells,
                      sizeof(struct llama_kv_cache_view_cell) * view->n_cells);
    GGML_ASSERT(p != nullptr && "Failed to alloc kv_cache_view cells");
    view->cells = (struct llama_kv_cache_view_cell *)p;
    p = realloc(view->cells_sequences,
                sizeof(llama_seq_id) * view->n_seq_max * view->n_cells);
    GGML_ASSERT(p != nullptr &&
                "Failed to alloc kv_cache_view cells sequences");
  ...
  for (int32_t i = 0; i < int32_t(ctx->kv_self.size);
       i++, c_curr++, cs_curr += view->n_seq_max) {
...
bool llama_kv_cache_seq_rm(struct llama_context *ctx, llama_seq_id seq_id,
...
void llama_kv_cache_seq_add(struct llama_context *ctx, llama_seq_id seq_id,
...
void llama_kv_cache_seq_div(struct llama_context *ctx, llama_seq_id seq_id,
...
llama_pos llama_kv_cache_seq_pos_max(struct llama_context *ctx,
                                     llama_seq_id seq_id) {
...
static inline ggml_tensor *get_kv_cache_split_tensor(const ggml_tensor *tensor,
                                                     const llama_layer &l) {
  bool use_V_for_K =
      l.attn_k_norm && l.attn_k_norm->ne[0] == l.wk->ne[1] ? true : false;
...
struct llama_data_write {
  ...
  virtual void write_tensor_data(const struct ggml_tensor *tensor,
                                 size_t offset, size_t size, int il) = 0;
  ...
  void write_logits(const struct llama_context *ctx) {
    const uint64_t logits_size =
        std::min((uint64_t)ctx->logits_size,
                 (uint64_t)ctx->n_outputs * ctx->model.hparams.n_vocab);
  ...
  void write_embeddings(const struct llama_context *ctx) {
    const uint64_t embeddings_size =
        std::min((uint64_t)ctx->embd_size,
                 (uint64_t)ctx->n_outputs * ctx->model.hparams.n_embd);
  ...
  void write_kv_cache_data(
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
      const uint32_t n_embd_k_gqa =
          hparams.n_embd_k_gqa(il) + hparams.n_embd_k_s();
      const uint32_t n_embd_head_qk_rope = hparams.n_rot;
      const uint32_t kv_lora_rank = hparams.n_lora_kv;
      ...
      // Read each range of cells of k_size length each into tmp_buf and write
      // out
      for (const auto &range : cell_ranges) {
    ...
    if (v_state == 0) {
      for (uint32_t il = 0; il < n_layer; ++il) {
        const uint32_t n_embd_v_gqa =
            hparams.n_embd_v_gqa(il) + hparams.n_embd_v_s();
        ...
        // Read each range of cells of v_size length each into tmp_buf and write
        // out
        for (const auto &range : cell_ranges) {
          const size_t range_size = range.second - range.first;
      ...
      // When v is transposed, we also need the element size and get the element
      // ranges from each row
      const uint32_t kv_size = kv_self.size;
      for (uint32_t il = 0; il < n_layer; ++il) {
        const uint32_t n_embd_v_gqa =
            hparams.n_embd_v_gqa(il) + hparams.n_embd_v_s();
        ...
        for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
          // Read each range of cells of v_size_el length each into tmp_buf and
          // write out
          for (const auto &range : cell_ranges) {
            const size_t range_size = range.second - range.first;
  ...
  void write_kv_cache(const struct llama_context *ctx,
                      llama_seq_id seq_id = -1) {
    const struct llama_kv_cache &kv_self = ctx->kv_self;
    std::vector<std::pair<uint32_t, uint32_t>>
        cell_ranges; // ranges, from inclusive, to exclusive
    ...
    // DEBUG CHECK: Sum of cell counts in ranges should equal the total cell
    // count
...
struct llama_data_read {
  ...
  void read_model_info(const struct llama_context *ctx) {
    ...
    if (cur_arch_str != arch_str) {
      throw std::runtime_error(format("wrong model arch: '%s' instead of '%s'",
                                      arch_str.c_str(), cur_arch_str.c_str()));
    }
    // TODO: add more info which needs to be identical but which is not verified
    // otherwise
  }
    ...
    if (n_outputs) {
      ...
      for (int32_t i = 0; i < (int32_t)output_pos.size(); ++i) {
        ...
        if ((uint32_t)id >= ctx->cparams.n_batch) {
          throw std::runtime_error(
              format("invalid output id, %d does not fit in batch size of %u",
                     id, ctx->cparams.n_batch));
        }
        ctx->output_ids[id] = i;
      }
  ...
  bool read_kv_cache_meta(struct llama_context *ctx, uint32_t cell_count,
    ...
    if (dest_seq_id != -1) {
      ...
      // DEBUG CHECK: kv_self.head should be our first cell, kv_self.head +
      // cell_count - 1 should be our last cell (verify seq_id and pos values)
      // Assume that this is one contiguous block of cells
      GGML_ASSERT(kv_self.head + cell_count <= kv_self.size);
      GGML_ASSERT(kv_self.cells[kv_self.head].pos == batch.pos[0]);
      GGML_ASSERT(kv_self.cells[kv_self.head + cell_count - 1].pos ==
                  batch.pos[cell_count - 1]);
  ...
  void read_kv_cache_data_split(llama_context *ctx, ggml_tensor *tensor,
                                const uint8_t *data, size_t head,
                                size_t row_size, int nrows, int il) {
    ...
    for (int id = 0; id < extra->n_device; ++id) {
      ...
      ggml_backend_tensor_set(split, aux.data(), head * split_row_size,
                              nrows * split_row_size);
  ...
  bool read_kv_cache_data(struct llama_context *ctx, uint32_t cell_count) {
    ...
    // Currently the only way there is no V cache (and thus v_state is 2)
    // requires flash_attn, and flash_attn sets kv_self.v_trans to false
    if (kv_self.v_trans != (v_state == 1)) {
      LLAMA_LOG_ERROR("%s: incompatible V transposition\n", __func__);
      return false;
    }

    // For each layer, read the keys for each cell, one row is one cell, read as
    ...
    for (uint32_t il = 0; il < n_layer; ++il) {
      ...
      if (cell_count) {
        ...
        if (kv_self.k_l[il]->extra) {
          read_kv_cache_data_split(ctx, kv_self.k_l[il],
                                   read(cell_count * k_size_row), kv_self.head,
                                   k_size_row, cell_count, il);
        } else {
          ggml_backend_tensor_set(
              kv_self.k_l[il], read(cell_count * k_size_row),
              kv_self.head * k_size_row, cell_count * k_size_row);
        }
      }
    }
    ...
    if (v_state == 0) {
      for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (cell_count) {
          ...
          if (kv_self.v_l[il]->extra) {
            read_kv_cache_data_split(ctx, kv_self.v_l[il],
                                     read(cell_count * v_size_row),
                                     kv_self.head, v_size_row, cell_count, il);
          } else {
            ggml_backend_tensor_set(
                kv_self.v_l[il], read(cell_count * v_size_row),
                kv_self.head * v_size_row, cell_count * v_size_row);
          }
        }
      }
    } else if (v_state == 1) {
      ...
      for (uint32_t il = 0; il < n_layer; ++il) {
        ...
        if (cell_count) {
          ...
          // For each row in the transposed matrix, read the values for the
          // whole cell range
          for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
            const size_t dst_offset =
                (kv_self.head + j * kv_self.size) * v_size_el;
            ggml_backend_tensor_set(kv_self.v_l[il],
                                    read(cell_count * v_size_el), dst_offset,
  ...
  void read_kv_cache(struct llama_context *ctx, llama_seq_id seq_id = -1) {
    ...
    bool res = read_kv_cache_meta(ctx, cell_count, seq_id) &&
               read_kv_cache_data(ctx, cell_count);
...
struct llama_data_write_buffer : llama_data_write {
  ...
  void write_tensor_data(const struct ggml_tensor *tensor, size_t offset,
                         size_t size, int il) override {
    if (size > buf_size) {
      throw std::runtime_error("unexpectedly reached end of buffer");
    }
    if (tensor->extra) {
  ...
  void get_tensor_data_split(const ggml_tensor *tensor, size_t offset,
                             size_t size, int il) {
    auto tt = ggml_internal_get_type_traits(tensor->type);
    if (tt.row_meta_size > 0) {
      throw std::runtime_error(std::string{"Split cache for type "} +
  ...
  static void get_tensor_data_split(uint8_t *ptr, const ggml_tensor *tensor,
    ...
    for (int id = 0; id < extra->n_device; ++id) {
      ...
      ggml_backend_tensor_get(split, aux_buffer.data(),
                              first_row * split_row_size, split_size);
...
struct llama_data_read_buffer : llama_data_read {
  ...
  llama_data_read_buffer(const uint8_t *p, size_t len)
      : ptr(p), buf_size(len) {}
  ...
  size_t get_size_read() override { return size_read; }
};
...
struct llama_data_write_file : llama_data_write {
  ...
  llama_data_write_file(llama_file *f, const llama_model &_model)
      : file(f), model(_model) {}
  ...
  void get_tensor_data_split(const struct ggml_tensor *tensor, size_t offset,
                             size_t size, int il) {
    GGML_ASSERT(il >= 0 && il < int(model.layers.size()));
    auto kv = get_kv_cache_split_tensor(tensor, model.layers[il]);
    temp_buffer.resize(size);
 ...
 *
 */
static size_t llama_state_get_data_internal(struct llama_context *ctx,
                                            llama_data_write &data_ctx) {
...
static size_t llama_state_set_data_internal(struct llama_context *ctx,
                                            llama_data_read &data_ctx) {
  llama_synchronize(ctx);
...
static bool llama_state_load_file_internal(struct llama_context *ctx,
                                           const char *path_session,
...
bool llama_state_load_file(struct llama_context *ctx, const char *path_session,
                           ...
                           size_t *n_token_count_out) {
  try {
    return llama_state_load_file_internal(ctx, path_session, tokens_out,
...
static bool llama_state_save_file_internal(struct llama_context *ctx,
                                           const char *path_session,
...
bool llama_state_save_file(struct llama_context *ctx, const char *path_session,
                           const llama_token *tokens, size_t n_token_count) {
  try {
    return llama_state_save_file_internal(ctx, path_session, tokens,
                                          n_token_count);
  } catch (const std::exception &err) {
...
static size_t llama_state_seq_get_data_internal(struct llama_context *ctx,
                                                llama_data_write &data_ctx,
                                                llama_seq_id seq_id) {
  llama_synchronize(ctx);
...
size_t llama_state_seq_get_data(struct llama_context *ctx, uint8_t *dst,
...
static size_t llama_state_seq_set_data_internal(struct llama_context *ctx,
                                                llama_data_read &data_ctx,
...
size_t llama_state_seq_set_data(struct llama_context *ctx, const uint8_t *src,
                                size_t size, llama_seq_id dest_seq_id) {
  llama_data_read_buffer data_ctx(src, size);
  try {
...
static size_t llama_state_seq_save_file_internal(struct llama_context *ctx,
                                                 const char *filepath,
                                                 llama_seq_id seq_id,
                                                 const llama_token *tokens,
                                                 size_t n_token_count) {
  ...
  GGML_ASSERT(res == sizeof(uint32_t) * 3 +
                         sizeof(llama_token) * n_token_count +
                         data_ctx.get_size_written());
  return res;
}

static size_t llama_state_seq_load_file_internal(struct llama_context *ctx,
                                                 const char *filepath,
                                                 llama_seq_id dest_seq_id,
                                                 llama_token *tokens_out,
                                                 size_t n_token_capacity,
  ...
  // load the prompt
  {
    ...
    if (n_token_count > n_token_capacity) {
      LLAMA_LOG_ERROR("%s: token count in sequence state file exceeded "
                      "capacity! %u > %zu\n",
                      __func__, n_token_count, n_token_capacity);
      return 0;
    }
  ...
  // restore the context state
  {
    ...
    llama_data_read_file data_ctx(&file);
    const size_t nread =
        llama_state_seq_set_data_internal(ctx, data_ctx, dest_seq_id);
    if (!nread) {
      LLAMA_LOG_ERROR("%s: failed to restore sequence state\n", __func__);
      return 0;
    }
    GGML_ASSERT(nread <= state_size);
    GGML_ASSERT(nread + sizeof(uint32_t) * 3 +
                    sizeof(llama_token) * *n_token_count_out ==
                file.tell());
  }
...
size_t llama_state_seq_save_file(struct llama_context *ctx,
                                 const char *filepath, llama_seq_id seq_id,
                                 const llama_token *tokens,
                                 size_t n_token_count) {
  try {
    return llama_state_seq_save_file_internal(ctx, filepath, seq_id, tokens,
                                              n_token_count);
  } catch (const std::exception &err) {
...
size_t llama_state_seq_load_file(struct llama_context *ctx,
                                 const char *filepath, llama_seq_id dest_seq_id,
                                 llama_token *tokens_out,
                                 size_t n_token_capacity,
                                 size_t *n_token_count_out) {
  try {
    return llama_state_seq_load_file_internal(ctx, filepath, dest_seq_id,
                                              tokens_out, n_token_capacity,
                                              n_token_count_out);
  } catch (const std::exception &err) {
...
struct llama_batch llama_batch_init(int32_t n_tokens_alloc, int32_t embd,
                                    int32_t n_seq_max) {
  llama_batch batch = {
  ...
  batch.seq_id =
      (llama_seq_id **)malloc(sizeof(llama_seq_id *) * (n_tokens_alloc + 1));
...
int32_t llama_decode(struct llama_context *ctx, struct llama_batch batch) {
  ...
  if (ctx->prefill_streaming && batch.n_tokens > 1) {
...
float *llama_get_logits_ith(struct llama_context *ctx, int32_t i) {
  ...
  try {
    ...
    if (i < 0) {
      ...
      if (j < 0) {
        throw std::runtime_error(
            format("negative index out of range [0, %d)", ctx->n_outputs));
      }
    ...
    if (j >= ctx->n_outputs) {
      ...
      throw std::runtime_error(format(
          "corrupt output buffer (j=%d, n_outputs=%d)", j, ctx->n_outputs));
    }
...
float *llama_get_embeddings_seq(struct llama_context *ctx,
                                llama_seq_id seq_id) {
  llama_synchronize(ctx);
...
enum llama_token_attr llama_token_get_attr(const struct llama_model *model,
                                           llama_token token) {
...
bool llama_token_is_control(const struct llama_model *model,
                            llama_token token) {
...
int32_t llama_tokenize(const struct llama_model *model, const char *text,
                       int32_t text_len, llama_token *tokens,
                       int32_t n_tokens_max, bool add_special,
                       bool parse_special) {
  return model->vocab.tokenize(text, text_len, tokens, n_tokens_max,
...
int32_t llama_detokenize(const struct llama_model *model,
                         const llama_token *tokens, int32_t n_tokens,
                         char *text, int32_t text_len_max, bool remove_special,
                         bool unparse_special) {
  return model->vocab.detokenize(tokens, n_tokens, text, text_len_max,
...
static llm_chat_template llama_chat_detect_template(const std::string &tmpl) {
  ...
  if (tmpl_contains("<|im_start|>")) {
  ...
  } else if (tmpl == "falcon_e" &&
             (tmpl_contains("assistant") && tmpl_contains("user"))) {
  ...
  } else if (tmpl_contains("<|START_OF_TURN_TOKEN|>") &&
  ...
  } else if (tmpl_contains(LU8("<｜Assistant｜>")) &&
             tmpl_contains(LU8("<｜User｜>")) &&
             tmpl_contains(LU8("<｜end▁of▁sentence｜>"))) {
    // original: if (tmpl_contains(LU8("'<｜Assistant｜>' + message['content'] +
    // '<｜end▁of▁sentence｜>'"))) {
    ...
    return LLM_CHAT_TEMPLATE_GRANITE;
                 ...
                 "message['role'] + additional_special_tokens[0] + "
                 "message['content'] + additional_special_tokens[1]")) {
    return LLM_CHAT_TEMPLATE_GIGACHAT;
  } else if (tmpl_contains("<|role_start|>")) {
    return LLM_CHAT_TEMPLATE_MEGREZ;
  } else if (tmpl_contains("<role>ASSISTANT</role>") &&
             tmpl_contains("'HUMAN'")) {
    return LLM_CHAT_TEMPLATE_BAILING;
  } else if (tmpl_contains("<role>ASSISTANT</role>") &&
             tmpl_contains("\"HUMAN\"") && tmpl_contains("<think>")) {
    return LLM_CHAT_TEMPLATE_BAILING_THINK;
  } else if (tmpl_contains("<role>ASSISTANT</role>") &&
             tmpl_contains("<role>HUMAN</role>") &&
             tmpl_contains("<|role_end|>")) {
    return LLM_CHAT_TEMPLATE_BAILING2;
  } else if (tmpl_contains("<|header_start|>") &&
             tmpl_contains("<|header_end|>")) {
...
static int32_t llama_chat_apply_template_internal(
    ...
    const std::vector<const llama_chat_message *> &chat, std::string &dest,
    bool add_ass) {
  ...
  if (tmpl == LLM_CHAT_TEMPLATE_CHATML) {
    ...
    for (auto message : chat) {
      ...
      if (role == "system") {
        ...
        ss << trailing_space
           << (trim_assistant_message ? trim(content) : content) << "</s>";
        is_inside_turn = false;
      }
    }
  } else if (tmpl == LLM_CHAT_TEMPLATE_LLAMA_2 ||
    ...
    for (auto message : chat) {
      ...
      if (role == "system") {
        if (support_system_message) {
          ...
          // if the model does not support system message, we still include it
          // in the first message, but without <<SYS>>
          ss << content << "\n";
        }
      } else if (role == "user") {
        ss << content << " [/INST]";
    ...
    for (auto message : chat) {
      std::string role(message->role);
      if (role == "system") {
        // there is no system message for gemma, but we will merge it with user
        // prompt, so nothing is broken
  ...
  } else if (tmpl == LLM_CHAT_TEMPLATE_VICUNA ||
             tmpl == LLM_CHAT_TEMPLATE_VICUNA_ORCA) {
    // eachadea/vicuna-13b-1.1 (and Orca variant)
    for (auto message : chat) {
      std::string role(message->role);
      if (role == "system") {
    ...
    for (auto message : chat) {
      std::string role(message->role);
      ss << "<|role_start|>" << role << "<|role_end|>" << message->content
         << "<|turn_end|>";
    }
    ...
    for (auto message : chat) {
      std::string role(message->role);
      if (role == "system") {
        ss << "<|im_system|>system<|im_middle|>" << message->content
           << "<|im_end|>";
      } else if (role == "assistant") {
        ss << "<|im_user|>user<|im_middle|>" << message->content
           << "<|im_end|>";
      } else {
        ss << "<|im_assistant|>assistant<|im_middle|>" << message->content
           << "<|im_end|>";
      }
    }
    if (add_ass) {
...
int32_t llama_chat_apply_template(const struct llama_model *model,
                                  const char *tmpl,
  ...
  if (tmpl == nullptr) {
    ...
    if (it != model->gguf_kv.end() && it->second.size() > 0) {
      ...
      // worst case: there is no information about template, we will use chatml
      // by default
  ...
  int32_t res = llama_chat_apply_template_internal(detected_tmpl, chat_vec,
...
void llama_grammar_sample(const struct llama_grammar *grammar,
                          const struct llama_context *ctx,
                          llama_token_data_array *candidates) {
  llama_grammar_sample_impl(grammar, &ctx->model.vocab, &ctx->sampling,
                            candidates);
}
...
void llama_grammar_accept_token(struct llama_grammar *grammar,
                                struct llama_context *ctx, llama_token token) {
...
void llama_sample_top_k(struct llama_context *ctx,
                        ...
                        size_t min_keep) {
  llama_sample_top_k_impl(ctx ? &ctx->sampling : nullptr, candidates, k,
                          min_keep);
}
...
void llama_sample_top_p(struct llama_context *ctx,
                        ...
                        size_t min_keep) {
  llama_sample_top_p_impl(ctx ? &ctx->sampling : nullptr, candidates, p,
                          min_keep);
}
...
void llama_sample_min_p(struct llama_context *ctx,
                        llama_token_data_array *candidates, float p,
                        size_t min_keep) {
  llama_sample_min_p_impl(ctx ? &ctx->sampling : nullptr, candidates, p,
                          min_keep);
}
...
void llama_sample_tail_free(struct llama_context *ctx,
                            ...
                            size_t min_keep) {
  llama_sample_tail_free_impl(ctx ? &ctx->sampling : nullptr, candidates, z,
                              min_keep);
}
...
void llama_sample_typical(struct llama_context *ctx,
                          llama_token_data_array *candidates, float p,
                          size_t min_keep) {
  llama_sample_typical_impl(ctx ? &ctx->sampling : nullptr, candidates, p,
                            min_keep);
}

void llama_sample_entropy(struct llama_context *ctx,
                          llama_token_data_array *candidates_p, float min_temp,
                          float max_temp, float exponent_val) {
  llama_sample_entropy_impl(ctx ? &ctx->sampling : nullptr, candidates_p,
                            min_temp, max_temp, exponent_val);
}
...
void llama_sample_xtc(struct llama_context *ctx,
                      llama_token_data_array *candidates_p, float probability,
                      float threshold, size_t min_keep) {
  llama_sample_xtc_impl(ctx ? &ctx->sampling : nullptr, candidates_p,
                        probability, threshold, min_keep);
}

void llama_sample_top_n_sigma(struct llama_context *ctx,
                              llama_token_data_array *candidates_p,
                              float top_n_sigma) {
  llama_sample_top_n_sigma_impl(ctx ? &ctx->sampling : nullptr, candidates_p,
                                top_n_sigma);
}

void llama_sample_dry([[maybe_unused]] struct llama_context *ctx,
                      struct llama_sampler_dry *smpl,
                      llama_token_data_array *candidates_p) {
...
void llama_sample_repetition_penalties(struct llama_context *ctx,
  ...
  llama_sample_repetition_penalties_impl(
      ctx ? &ctx->sampling : nullptr, candidates, last_tokens, penalty_last_n,
      penalty_repeat, penalty_freq, penalty_present);
}

void llama_sample_apply_guidance(struct llama_context *ctx, float *logits,
                                 float *logits_guidance, float scale) {
  llama_sample_apply_guidance_impl(&ctx->sampling, logits, logits_guidance,
                                   scale);
}

llama_token llama_sample_token_mirostat(struct llama_context *ctx,
                                        llama_token_data_array *candidates,
                                        float tau, float eta, int32_t m,
                                        float *mu) {
  return llama_sample_token_mirostat_impl(&ctx->sampling, candidates, tau, eta,
                                          m, mu);
}
...
llama_token llama_sample_token_mirostat_v2(struct llama_context *ctx,
  ...
  return llama_sample_token_mirostat_v2_impl(ctx ? &ctx->sampling : nullptr,
                                             candidates, tau, eta, mu);
}

llama_token llama_sample_token_greedy(struct llama_context *ctx,
                                      llama_token_data_array *candidates) {
  return llama_sample_token_greedy_impl(ctx ? &ctx->sampling : nullptr,
                                        candidates);
}
...
llama_token llama_sample_token(struct llama_context *ctx,
                               llama_token_data_array *candidates) {
  return llama_sample_token_with_rng_impl(&ctx->sampling, candidates,
                                          ctx->sampling.rng);
}

llama_token
llama_sample_token_adaptive_p(struct llama_context *ctx,
                              llama_token_data_array *candidates,
                              struct llama_sampler_adaptive_p *adapt_p_ctx) {
  return llama_sample_token_adaptive_p_impl(&ctx->sampling, candidates,
                                            adapt_p_ctx);
}

int llama_split_path(char *split_path, size_t maxlen, const char *path_prefix,
                     int split_no, int split_count) {
  static const char *const SPLIT_PATH_FORMAT = "%s-%05d-of-%05d.gguf";
  if (snprintf(split_path, maxlen, SPLIT_PATH_FORMAT, path_prefix, split_no + 1,
               split_count)) {
...
llama_sampler_init_dry(const struct llama_vocab *vocab, float dry_multiplier,
                       float dry_base, int32_t dry_allowed_length,
                       int32_t dry_penalty_last_n, const char **seq_breakers,
                       size_t num_breakers) {
  return llama_sampler_init_dry_impl(
      *vocab, vocab->n_tokens(), dry_multiplier, dry_base, dry_allowed_length,
...
void llama_sampler_dry_free(struct llama_sampler_dry *smpl) { delete smpl; }

struct llama_sampler_dry *
llama_sampler_dry_clone(struct llama_sampler_dry *smpl) {
  // nullptr is passed as vocab because it is only needed for raw sequence
  // breaker processing, which we have already done and will be copying
  auto *result = llama_sampler_init_dry(
      nullptr, smpl->dry_multiplier, smpl->dry_base, smpl->dry_allowed_length,
      smpl->dry_penalty_last_n, NULL, 0);
...
void llama_sampler_dry_accept(struct llama_sampler_dry *smpl,
                              llama_token token) {
  if (!smpl) {
    return;
  }
  if (smpl->dry_multiplier == 0.0f || smpl->dry_base < 1.0f ||
      smpl->dry_penalty_last_n == 0) {
    return;
  }
...
llama_init_adaptive_p(int n_vocab, const float target, const float decay,
...
int llama_split_prefix(char *dest, size_t maxlen, const char *split_path,
  ...
  if (size_prefix > 0 &&
      str_split_path.find(str_postfix, size_prefix) != std::string::npos) {
...
#elif defined(GGML_USE_CUDA)
  ggml_backend_cuda_log_set_callback(g_state.log_callback,
                                     g_state.log_callback_user_data);
#elif defined(GGML_USE_CANN)
  ggml_backend_cann_log_set_callback(g_state.log_callback,
                                     g_state.log_callback_user_data);
#endif
}
...
void llama_log_callback_default(ggml_log_level level, const char *text,
                                void *user_data) {
  (void)level;
  (void)user_data;
  fputs(text, stderr);
...
void llama_set_offload_policy(struct llama_context *lctx, int op,
                              bool on_or_off) {
  if (!lctx || !lctx->sched)
    return;
  const char *op_name = op < 0 || op >= int(GGML_OP_COUNT)
                            ? "all ops"
                            : ggml_op_name(ggml_op(op));
