...
enum llama_vocab_type {
  ...
  LLAMA_VOCAB_TYPE_PLAMO2 =
      6, // PLaMo-2 tokenizer based on Aho-Corasick with dynamic programming
};
...
enum llama_ftype {
  ...
  LLAMA_FTYPE_MOSTLY_MXFP4 =
      38, // except 1d tensors, 38 to be compatible with mainline
  //
...
enum llama_prefill_decode_mode {
  ...
  LLAMA_PREFILL_DECODE_MODE_HYBRID =
      3, // mixed decode placement (policy-defined)
  LLAMA_PREFILL_DECODE_MODE_SPLIT_THUNDERBOLT =
      4, // mixed decode split intended for TB transport path
};
...
struct llama_model_params {
  ...
  int32_t mla; // MLA implementation to use (only applicable to DeepSeek models
               // at this point)
  enum llama_split_mode
  ...
  bool merge_qkv; // if true, merge separate Q, K, V tensors into a single,
                  // contiguous tensor
  bool merge_up_gate_exps; // if true, merge ffn_up_exps and ffn_gate_exps
                           // tensors into a single, contiguous tensor
};
...
struct llama_context_params {
  ...
  int32_t max_extra_alloc;  // Max. additional VRAM the scheduler is allowed to
                            // allocate
  ...
  int attn_max_batch;          // maximum batch size for attention computations
                               // [EXPERIMENTAL]
  bool fused_moe_up_gate;      // whether to use fused MoE up/gate op
  bool grouped_expert_routing; // whether to use grouped expert routing
                               // (BailingMoeV2 arch)
  ...
  bool
      split_mode_graph_scheduling; // if true, force split mode graph scheduling
  // bool split_mode_f16;    // if true, cast intermediate results to f16 before
  // copying to other GPUs
  bool scheduler_async; // if true, with split mode "graph" graph evaluation
                        // will be done using multiple threads
...
typedef struct llama_model_quantize_params {
  ...
  enum ggml_type ffn_gate_inp_type;    // routed experts probabilities typy
                                       // (relevant for MoE models only)
  ...
  bool ignore_imatrix_rules; // If set to true, the built-in rules for refusing
                             // to quantize into certain quants without imatrix
                             // are ignored
  ...
  void *repack_pattern; // pointer to a vector containing regexes to be used for
                        // matching tensor names. Can be null
...
LLAMA_API void llama_set_offload_policy(struct llama_context *lctx, int op,
                                        bool on_or_off);
...
LLAMA_API const struct llama_vocab *
...
// The loaded adapter will be associated to the given model, and will be free
// when the model is deleted
LLAMA_API struct llama_lora_adapter *
llama_lora_adapter_init(struct llama_model *model, const char *path_lora);

// Add a loaded LoRA adapter to given context
// This will not modify model's weight
LLAMA_API int32_t llama_lora_adapter_set(struct llama_context *ctx,
                                         struct llama_lora_adapter *adapter,
                                         float scale);

// Remove a specific LoRA adapter from given context
// Return -1 if the adapter is not present in the context
LLAMA_API int32_t llama_lora_adapter_remove(struct llama_context *ctx,
                                            struct llama_lora_adapter *adapter);
...
llama_token_get_attr(const struct llama_model *model, llama_token token);
...
LLAMA_API bool llama_token_is_control(const struct llama_model *model,
                                      llama_token token);
...
LLAMA_API llama_token
llama_token_pad(const struct llama_model *model); // padding
...
LLAMA_API int32_t llama_vocab_tokenize(const struct llama_vocab *vocab,
                                       const char *text, int32_t text_len,
                                       llama_token *tokens,
...
LLAMA_API int32_t llama_token_to_piece(const struct llama_model *model,
                                       ...
                                       bool special);
LLAMA_API int32_t llama_vocab_token_to_piece(const struct llama_vocab *vocab,
                                             llama_token token, char *buf,
                                             int32_t length, int32_t lstrip,
                                             bool special);
...
LLAMA_API int32_t llama_detokenize(const struct llama_model *model,
...
struct llama_sampler_i {
  ...
  struct llama_sampler *(*clone)(
      const struct llama_sampler *);        // can be NULL if ctx is NULL
...
/// @param start_rule_index The index of the root rule (the starting point of
/// the grammar).
...
LLAMA_API void llama_grammar_sample(const struct llama_grammar *grammar,
                                    const struct llama_context *ctx,
                                    llama_token_data_array *candidates);
LLAMA_API
DEPRECATED(void llama_sample_grammar(struct llama_context *ctx,
                                     llama_token_data_array *candidates,
                                     const struct llama_grammar *grammar),
           "use llama_grammar_sample instead");

/// @details Accepts the sampled token into the grammar
LLAMA_API void llama_grammar_accept_token(struct llama_grammar *grammar,
...
LLAMA_API void llama_sample_xtc(struct llama_context *ctx,
                                llama_token_data_array *candidates_p,
...
LLAMA_API void llama_sample_top_n_sigma(struct llama_context *ctx,
                                        llama_token_data_array *candidates_p,
                                        float top_n_sigma);
...
/// @param grammar_str The production rules for the grammar, encoded as a
/// string. Returns an empty grammar if empty. Returns NULL if parsing of
/// grammar_str fails.
/// @param grammar_root The name of the start symbol for the grammar.
LLAMA_API struct llama_grammar *
llama_sampler_init_grammar(const struct llama_vocab *vocab,
...
/// @param trigger_patterns A list of patterns that will trigger the grammar
/// sampler. Pattern will be matched from the start of the generation output,
/// and grammar sampler will be fed content starting from its first match group.
/// @param trigger_tokens A list of tokens that will trigger the grammar
/// sampler. Grammar sampler will be fed content starting from the trigger token
/// included.
...
LLAMA_API struct llama_sampler_dry *
llama_sampler_init_dry(const struct llama_vocab *model, float dry_multiplier,
                       float dry_base, int32_t dry_allowed_length,
                       int32_t dry_penalty_last_n, const char **seq_breakers,
                       size_t num_breakers);

// LLAMA_API void llama_sample_dry(struct llama_context* ctx,
// llama_token_data_array* candidates_p, int32_t context_size, float
// dry_multiplier, float dry_base, int32_t dry_allowed_length, int32_t
// dry_penalty_last_n, const char** seq_breakers, size_t num_breakers);

void llama_sample_dry(struct llama_context *ctx, struct llama_sampler_dry *smpl,
                      llama_token_data_array *candidates_p);
...
/// @param target Select tokens near this probability (valid range 0.0 to 1.0;
/// <0 = disabled)
/// @param decay Decay rate for target adaptation over time. lower values ->
/// faster but less stable adaptation. (valid range 0.0 to 1.0; ≤0 = no
/// adaptation)
LLAMA_API struct llama_sampler_adaptive_p *
...
/// @details Adaptive p sampler described in
/// https://github.com/MrJackSpade/adaptive-p-docs/blob/main/README.md
...
/// @details Randonly selects a token from the candidates following adaptive p
/// sampler.
llama_token
