...
static typename std::enable_if<
    !std::is_same<T, bool>::value && std::is_integral<T>::value, void>::type
get_env(std::string name, T &target) {
  char *value = std::getenv(name.c_str());
...
static std::string add_rpc_devices(std::string &servers) {
  ...
  if (!rpc_devices.empty()) {
    rpc_devices =
        rpc_devices.substr(0, rpc_devices.size() - 1); // remove trailing comma
  }
#endif
  return rpc_devices;
}
...
common_remote_get_content(const std::string &url,
                          const common_remote_params &) {
  if (!url.empty()) {
    throw std::runtime_error(
        "error: built without CURL, cannot download file from the internet");
  }
  return {};
}
...
void gpt_params_handle_model_default(gpt_params &params) {
  if (!params.hf_repo.empty()) {
    ...
    if (params.hf_file.empty()) {
      ...
      params.model =
          fs_get_cache_file(string_split(params.hf_file, "/").back());
    }
...
bool gpt_params_parse_ex(int argc, char **argv, gpt_params &params) {
  ...
  if (!params.chat_template.empty() &&
    ...
    throw std::runtime_error(string_format(
        ...
        params.use_jinja ? ""
                         : "\nnote: llama.cpp was started without --jinja, we "
                           "only support commonly used templates"));
  }
  return true;
}
...
void gpt_params_parse_from_env(gpt_params &params) {
  ...
  get_env("LLAMA_ARG_KV_RECV_NACK_ON_CRC_BAD",
          params.kv_receiver_nack_on_crc_bad);
  get_env("LLAMA_ARG_KV_RECV_MAX_CONNECTIONS",
          params.kv_receiver_max_connections);
  get_env("LLAMA_ARG_KV_RECV_IDLE_TIMEOUT",
          params.kv_receiver_idle_timeout_sec);
  get_env("LLAMA_ARG_KV_RECV_STALE_FINALIZE_TIMEOUT",
          params.kv_receiver_stale_finalize_timeout_sec);
  get_env("LLAMA_ARG_KV_RECV_SESSION_RETENTION",
          params.kv_receiver_session_retention_sec);
  get_env("LLAMA_ARG_KV_RECV_CLEANUP_INTERVAL",
          params.kv_receiver_cleanup_interval_sec);
  get_env("LLAMA_ARG_KV_RECV_SOCKET_SEND_BUF",
  ...
  get_env("LLAMA_ARG_PREFILL_TRANSPORT_CHUNK_BYTES",
          params.prefill_transport_chunk_bytes);
  get_env("LLAMA_ARG_PREFILL_DECODE_TRANSPORT_REQUIRED",
          params.prefill_decode_transport_required);
  get_env("LLAMA_ARG_PREFILL_TRANSPORT_SESSION_DIR",
          params.prefill_transport_session_dir);
  get_env("LLAMA_ARG_PREFILL_MIN_STREAM_BATCH_TOKENS",
...
std::vector<std::pair<T1, T2>> string_split_pairs(const std::string &str,
                                                  char delim) {
...
bool gpt_params_find_arg(int argc, char **argv, const std::string &arg,
  ...
  if (arg == "-s" || arg == "--seed") {
    CHECK_ARG
    // TODO: this is temporary, in the future the sampling state will be moved
    // fully to llama_sampling_context.
  ...
  if (arg == "--dry-sequence-breaker") {
    ...
    if (value == "none") {
      ...
      for (size_t i = 0; i < value.size(); i++) {
        params.sparams.dry_sequence_breakers.emplace_back(std::string{} +
                                                          value[i]);
      }
    }
    return true;
  ...
  if (arg == "-ngld" || arg == "--gpu-layers-draft" ||
      arg == "--n-gpu-layers-draft") {
    CHECK_ARG
    params.n_gpu_layers_draft = std::stoi(argv[i]);
    if (!llama_supports_gpu_offload()) {
      fprintf(stderr, "warning: not compiled with GPU offload support, "
                      "--gpu-layers-draft option will be ignored\n");
      fprintf(stderr, "warning: see main README.md for information on enabling "
                      "GPU BLAS support\n");
    }
    return true;
  ...
  if (arg == "--override-tensor" || arg == "-ot") {
    CHECK_ARG
    if (!parse_buft_overrides(std::string{argv[i]},
  ...
  if (arg == "--cpu-moe" || arg == "-cmoe") {
    params.tensor_buft_overrides.push_back(
        {strdup("\\.ffn_(up|down|gate)_exps\\.weight"),
  ...
  if (arg == "--n-cpu-moe" || arg == "-ncmoe") {
    ...
    if (n_layers < 0) {
      fprintf(stderr,
              "error: Invalid value for --n-cpu-moe: %d (must be >= 0)\n",
              n_layers);
      invalid_param = true;
      return true;
    }
  ...
  if (arg == "--prefill-decode-transport-required" ||
      arg == "--decode-transport-required") {
  ...
  if (arg == "--prefill-decode-remote-layers" ||
      arg == "--decode-remote-layers") {
    CHECK_ARG
  ...
  if (arg == "--prefill-decode-remote-ranges" ||
      arg == "--decode-remote-ranges") {
    CHECK_ARG
  ...
  if (arg == "--offload-policy" || arg == "-op") {
    ...
    if (p.empty()) {
      ...
      params.offload_policy.insert(params.offload_policy.end(), p.begin(),
                                   p.end());
    }
    return true;
  }
  ...
  if (arg == "--chat-template") {
    ...
    if (!common_chat_verify_template(argv[i], true)) {
      fprintf(stderr,
              ...
              argv[i]);
      fprintf(stderr, "note: llama.cpp does not use jinja parser, we only "
                      "support commonly used templates\n");
      invalid_param = true;
      return true;
    }
  ...
  if (arg == "--output-format") {
    ...
    std::string value(argv[i]);
    /**/ if (value == "jsonl") {
      params.sweep_bench_output_jsonl = true;
    } else if (value == "md") {
      params.sweep_bench_output_jsonl = false;
    } else {
      invalid_param = true;
    }
    return true;
...
void gpt_params_print_usage(int /*argc*/, char **argv,
  ...
  struct option_info {
    ...
    option_info(const std::string &tags, const char *args, const char *desc,
                ...)
  ...
  options.push_back({"*", "       --verbosity N",
                     "set specific verbosity level (default: %d)",
                     params.verbosity});
  options.push_back({"*", "       --verbose-prompt",
                     "print a verbose prompt before generation (default: %s)",
                     params.verbose_prompt ? "true" : "false"});
  options.push_back({"*", "       --no-display-prompt",
                     "don't print prompt at generation (default: %s)",
                     !params.display_prompt ? "true" : "false"});
  options.push_back({"*", "-co,   --color",
                     "colorise output to distinguish prompt and user input "
                     "from generations (default: %s)",
                     params.use_color ? "true" : "false"});
  options.push_back({"*", "-s,    --seed SEED",
                     "RNG seed (default: %d, use random seed for < 0)",
                     params.seed});
  options.push_back({"*", "-t,    --threads N",
                     "number of threads to use during generation (default: %d)",
                     params.n_threads});
  options.push_back({"*", "-tb,   --threads-batch N",
                     "number of threads to use during batch and prompt "
                     "processing (default: same as --threads)"});
  options.push_back({"speculative", "-td,   --threads-draft N",
                     "number of threads to use during generation (default: "
                     "same as --threads)"});
  options.push_back({"speculative", "-tbd,  --threads-batch-draft N",
                     "number of threads to use during batch and prompt "
                     "processing (default: same as --threads-draft)"});
  options.push_back({"speculative", "-ps,   --p-split N",
                     "speculative decoding split probability (default: %.1f)",
                     (double)params.p_split});
  options.push_back({"*", "-lcs,  --lookup-cache-static FNAME",
                     "path to static lookup cache to use for lookup decoding "
                     "(not updated by generation)"});
  options.push_back({"*", "-lcd,  --lookup-cache-dynamic FNAME",
                     "path to dynamic lookup cache to use for lookup decoding "
                     "(updated by generation)"});

  options.push_back(
      {"*", "-c,    --ctx-size N",
       "size of the prompt context (default: %d, 0 = loaded from model)",
       params.n_ctx});
  options.push_back({"*", "-cd,   --ctx-size-draft N",
                     "size of the prompt context for the draft model (default: "
                     "%d, 0 = loaded from model)",
                     params.n_ctx_draft});
  options.push_back({"*", "-cram, --cache-ram N",
                     "set the maximum cache size in MiB (default: %d, -1 - no "
                     "limit, 0 - disable)",
                     params.cache_ram_mib});
  options.push_back({"*", "-crs,  --cache-ram-similarity N",
                     "max of similarity of prompt tokens to cache tokens that "
                     "triggers prompt cache (default: %.2f).",
                     params.cache_ram_similarity});
  options.push_back({"*", "-cram-n-min --cache-ram-n-min N",
                     "minimum number of the cached tokens that triggers prompt "
                     "cache (default: %d).",
                     params.cache_ram_n_min});
  options.push_back({"*", "-n,    --predict N",
                     "number of tokens to predict (default: %d, -1 = infinity, "
                     "-2 = until context filled)",
                     params.n_predict});
  options.push_back({"*", "-b,    --batch-size N",
                     "logical maximum batch size (default: %d)",
                     params.n_batch});
  options.push_back({"*", "-ub,   --ubatch-size N",
                     "physical maximum batch size (default: %d)",
                     params.n_ubatch});
  options.push_back({"*", "       --keep N",
                     "number of tokens to keep from the initial prompt "
                     "(default: %d, -1 = all)",
                     params.n_keep});
  options.push_back({"*", "       --chunks N",
                     "max number of chunks to process (default: %d, -1 = all)",
                     params.n_chunks});
  options.push_back({"*", "-no-fa, --no-flash-attn",
                     "disable Flash Attention (default: %s)",
                     params.flash_attn ? "enabled" : "disabled"});
  options.push_back({"*", "-fa, --flash-attn (auto|on|off|0|1)",
                     "set Flash Attention (default: %s)",
                     params.flash_attn ? "on" : "off"});
  options.push_back(
      {"*", "-mla,  --mla-use", "enable MLA (default: %d)", params.mla_attn});
  options.push_back({"*", "-amb,  --attention-max-batch",
                     "max batch size for attention computations (default: %d)",
                     params.attn_max_batch});
  options.push_back({"*", "-no-fmoe, --no-fused-moe",
                     "disable fused MoE (default: %s)",
                     params.fused_moe_up_gate ? "enabled" : "disabled"});
  options.push_back({"*", "-ger,  --grouped-expert-routing",
                     "enable grouped expert routing (default: %s)",
                     params.grouped_expert_routing ? "enabled" : "disabled"});
  options.push_back({"*", "-no-fug, --no-fused-up-gate",
                     "disaable fused up-gate (default: %s)",
                     params.fused_up_gate ? "enabled" : "disabled"});
  options.push_back({"*", "-no-mmad, --no-fused-mul-multiadd",
                     "disable fused mul-multi_add (default: %s)",
                     params.fused_mmad ? "enabled" : "disabled"});
  // options.push_back({ "*",           "-rcache, --rope-cache",         "enable
  // RoPE cache (default: %s)", params.rope_cache ? "enabled" : "disabled" });
  options.push_back({"*", "-gr, --graph-reuse",
                     "enable graph reuse (default: %s)",
                     params.graph_reuse ? "enabled" : "disabled"});
  options.push_back({"*", "-no-gr, --no-graph-reuse",
                     "disable graph reuse (default: %s)",
                     !params.graph_reuse ? "enabled" : "disabled"});
  options.push_back({"*", "-ser,  --smart-expert-reduction",
                     "experts reduction (default: %d,%g)", params.min_experts,
                     params.thresh_experts});
  options.push_back({"*", "-mqkv,  --merge-qkv,", "merge Q,K,V (default: %d)",
                     params.merge_qkv});
  options.push_back({"*", "-muge,  --merge-up-gate-experts,",
                     "merge ffn_up/gate_exps (default: %d)",
                     params.merge_up_gate_exps});
  options.push_back({"*", "-khad,  --k-cache-hadamard,",
                     "Use Hadamard transform for K-cache (default: %d)",
                     params.k_cache_hadamard});
  options.push_back({"*", "-smf16, --split-mode-f16,",
                     "Use f16 for data exchange between GPUs (default: %d)",
                     true});
  options.push_back({"*", "-smf32, --split-mode-f32,",
                     "Use f32 for data exchange between GPUs (default: %d)",
                     false});
  options.push_back({"*", "-grt, --graph-reduce-type",
                     "Type for data exchange between GPUs (default: %s)",
                     "f32"});
  options.push_back({"*", "-smgs, --split-mode-graph-scheduling,",
                     "Force Split Mode Graph Scheduling (default: %d)",
                     params.split_mode_graph_scheduling});
  options.push_back({"*", "-sas,  ==scheduler_async,",
                     "Async evaluation of compute graphs: %d)",
                     params.scheduler_async});
  options.push_back(
      {"*", "-vq, --validate-quants",
       "validate quantized data while loading the model (default: %d)",
  ...
  options.push_back(
      {"*", "-p,    --prompt PROMPT",
       ...
       "(default: '%s')",
       params.prompt.c_str()});
  options.push_back({"*", "-f,    --file FNAME",
                     "a file containing the prompt (default: none)"});
  options.push_back({"*", "       --in-file FNAME",
                     "an input file (repeat to specify multiple files)"});
  options.push_back({"*", "-bf,   --binary-file FNAME",
                     "binary file containing the prompt (default: none)"});
  options.push_back({"*", "-e,    --escape",
                     "process escapes sequences (\\n, \\r, \\t, \\', \\\", "
                     "\\\\) (default: %s)",
                     params.escape ? "true" : "false"});
  options.push_back(
      {"*", "       --no-escape", "do not process escape sequences"});
  options.push_back({"main", "-ptc,  --print-token-count N",
                     "print token count every N tokens (default: %d)",
                     params.n_print});
  options.push_back(
      {"main", "       --prompt-cache FNAME",
       "file to cache prompt state for faster startup (default: none)"});
  options.push_back(
      {"main", "       --prompt-cache-all",
       "if specified, saves user input and generations to cache as well\n"
       "not supported with --interactive or other interactive options"});
  options.push_back(
      {"main", "       --prompt-cache-ro",
       "if specified, uses the prompt cache but does not update it"});
  options.push_back(
      {"main", "-r,    --reverse-prompt PROMPT",
       "halt generation at PROMPT, return control in interactive mode\n"
       "can be specified more than once for multiple prompts"});
  options.push_back({"main", "-sp,   --special",
                     "special tokens output enabled (default: %s)",
                     params.special ? "true" : "false"});
  options.push_back({"main", "-cnv,  --conversation",
                     "run in conversation mode, does not print special tokens "
                     "and suffix/prefix\n"
                     "if suffix/prefix are not specified, default chat "
                     "template will be used\n"
                     "(default: %s)",
                     params.conversation ? "true" : "false"});
  options.push_back({"main infill", "-i,    --interactive",
                     "run in interactive mode (default: %s)",
                     params.interactive ? "true" : "false"});
  options.push_back(
      {"main infill", "-if,   --interactive-first",
       "run in interactive mode and wait for input right away (default: %s)",
       params.interactive_first ? "true" : "false"});
  options.push_back({"main infill", "-mli,  --multiline-input",
                     "allows you to write or paste multiple lines without "
                     "ending each in '\\'"});
  options.push_back(
      {"main infill", "       --in-prefix-bos",
       "prefix BOS to user inputs, preceding the `--in-prefix` string"});
  options.push_back({"main infill", "       --in-prefix STRING",
                     "string to prefix user inputs with (default: empty)"});
  options.push_back(
      {"main infill", "       --in-suffix STRING",
       "string to suffix after user inputs with (default: empty)"});
  options.push_back({"main", "       --no-warmup",
                     "skip warming up the model with an empty run"});
  options.push_back(
      {"server infill", "       --spm-infill",
       "use Suffix/Prefix/Middle pattern for infill (instead of "
       "Prefix/Suffix/Middle) as some models prefer this. (default: %s)",
       params.spm_infill ? "enabled" : "disabled"});

  options.push_back({"sampling"});
  options.push_back({"*", "       --samplers SAMPLERS",
                     "samplers that will be used for generation in the order, "
                     "separated by \';\'\n"
                     "(default: %s)",
  ...
  options.push_back({"*", "       --ignore-eos",
                     "ignore end of stream token and continue generating "
                     "(implies --logit-bias EOS-inf)"});
  options.push_back({"*", "       --penalize-nl",
                     "penalize newline tokens (default: %s)",
                     sparams.penalize_nl ? "true" : "false"});
  options.push_back({"*", "       --temp N", "temperature (default: %.1f)",
                     (double)sparams.temp});
  options.push_back({"*", "       --top-k N",
                     "top-k sampling (default: %d, 0 = disabled)",
                     sparams.top_k});
  options.push_back({"*", "       --top-p N",
                     "top-p sampling (default: %.1f, 1.0 = disabled)",
                     (double)sparams.top_p});
  options.push_back({"*", "       --min-p N",
  ...
  options.push_back({"*", "       --repeat-last-n N",
                     "last n tokens to consider for penalize (default: %d, 0 = "
                     "disabled, -1 = ctx_size)",
  ...
  options.push_back({"*", "       --dynatemp-exp N",
  ...
  options.push_back(
      {"*", "       --mirostat N",
       ...
       sparams.mirostat});
  options.push_back({"*", "       --mirostat-lr N",
                     "Mirostat learning rate, parameter eta (default: %.1f)",
                     (double)sparams.mirostat_eta});
  options.push_back({"*", "       --mirostat-ent N",
                     "Mirostat target entropy, parameter tau (default: %.1f)",
                     (double)sparams.mirostat_tau});
  options.push_back({"*", "       --xtc-probability p",
                     "xtc probability (default: %.1f, 0.0 = disabled)",
                     (double)sparams.xtc_probability});
  options.push_back({"*", "       --xtc-threshold t",
                     "xtc threshold (default: %.1f, >0.5 = disabled)",
                     (double)sparams.xtc_threshold});
  options.push_back({"*", "       --top-n-sigma t",
                     "top-n-sigma parmeter (default: %.1f, 0.0 = disabled)",
                     (double)sparams.top_n_sigma});
  options.push_back({"*", "       --adaptive-target",
                     "adaptive-p sampling: (default: %.2f, <0.0 = disabled)",
                     (double)sparams.adaptive_target});
  options.push_back({"*", "       --adaptive-decay",
                     "adaptive-p sampling: (default: %.2f)",
                     (double)sparams.adaptive_decay});
  options.push_back({"*", "       --banned-string-file",
                     "file path of the list of banned strings on each line"});
  options.push_back({"*", "       --banned-n",
                     "number of tokens banned in the phrase during rewind. -1 "
                     "means all tokens: (default: %d)",
                     params.banned_n});
  options.push_back({"*", "       --adaptive-updt-w-cur",
                     "adaptive-p sampling: (default: %s)",
  ...
  options.push_back({"main", "       --cfg-scale N",
                     "strength of guidance (default: %.1f, 1.0 = disable)",
                     (double)sparams.cfg_scale});
  ...
  options.push_back({"main",
                     "       --chat-template-file file_with_JINJA_TEMPLATE",
                     "load jinja template for chat from the file\n"});
  options.push_back({
      "main",
      "       --reasoning-format FORMAT",
      "controls whether thought tags are allowed and/or extracted from the "
  ...
  options.push_back({"main", "       --chat-template-kwargs JSON",
                     "sets additional params for the json template parser"});
  options.push_back({"main", "       --reasoning-budget N",
                     "controls the amount of thinking allowed.\n"
                     "currently only one of: -1 for unrestricted thinking "
                     "budget, or 0 to disable thinking"
                     "(default: -1)"});
  options.push_back(
      {"main", "       --reasoning-tokens FORMAT",
  ...
  options.push_back({"prefill", "       --prefill-prefetch N",
                     "prefill layer lookahead distance (default: %d)",
                     params.prefill_prefetch});
  options.push_back({"prefill", "       --prefill-slab-bytes N",
                     "prefill upload slab bytes (default: %zu)",
                     params.prefill_slab_bytes});
  ...
  options.push_back({"prefill", "       --prefill-transport-mode MODE",
                     ...
                     "(default: %s)",
                     params.prefill_transport_mode.c_str()});
  options.push_back({"prefill", "       --prefill-execution-mode MODE",
                     "prefill execution mode: coupled|decoupled (default: %s)",
  ...
  options.push_back({"prefill", "       --prefill-decode-gpu-layers N",
                     "decode GPU layer hint for hybrid mode (default: %d)",
                     params.decode_gpu_layers_hint});
  options.push_back({"prefill", "       --prefill-decode-remote-layers N",
                     "decode remote layer hint for split mode (default: %d)",
                     params.decode_remote_layers_hint});
  ...
  options.push_back({"prefill", "       --kv-max-inflight-bytes N",
                     "sender max in-flight bytes override"});
  options.push_back({"prefill", "       --kv-socket-send-buf N",
                     "sender socket SO_SNDBUF override"});
  options.push_back({"prefill", "       --kv-socket-recv-buf N",
                     "sender socket SO_RCVBUF override"});
  options.push_back({"prefill", "       --kv-bind-addrs CSV",
                     "sender bind-address list override"});
  options.push_back({"prefill", "       --kv-peer-addrs CSV",
                     "sender peer-address list override"});
  options.push_back({"prefill", "       --kv-balance MODE",
                     "sender multi-peer balancing policy override"});
  options.push_back({"prefill", "       --tb-direct-endpoint ENDPOINT",
                     "sender direct endpoint override for TB transport"});
  options.push_back({"grammar"});
  options.push_back({"*", "       --grammar GRAMMAR",
                     "BNF-like grammar to constrain generations (see samples "
                     "in grammars/ dir) (default: '%s')",
                     sparams.grammar.c_str()});
  options.push_back(
  ...
  options.push_back(
      {"embedding", "       --pooling {none,mean,cls,last}",
       "pooling type for embeddings, use model default if unspecified"});
  options.push_back(
      {"embedding", "       --attention {causal,non-causal}",
  ...
  options.push_back({"*", "       --rope-freq-base N",
                     "RoPE base frequency, used by NTK-aware scaling (default: "
                     "loaded from model)"});
  options.push_back(
      {"*", "       --rope-freq-scale N",
       "RoPE frequency scaling factor, expands context by a factor of 1/N"});
  options.push_back({"*", "       --yarn-orig-ctx N",
                     "YaRN: original context size of model (default: %d = "
                     "model training context size)",
                     params.yarn_orig_ctx});
  options.push_back({"*", "       --yarn-ext-factor N",
                     "YaRN: extrapolation mix factor (default: %.1f, 0.0 = "
                     "full interpolation)",
  ...
  options.push_back({"*", "       --yarn-beta-slow N",
                     "YaRN: high correction dim or alpha (default: %.1f)",
                     (double)params.yarn_beta_slow});
  options.push_back({"*", "       --yarn-beta-fast N",
                     "YaRN: low correction dim or beta (default: %.1f)",
                     (double)params.yarn_beta_fast});
  options.push_back({"*", "-gan,  --grp-attn-n N",
                     "group-attention factor (default: %d)",
                     params.grp_attn_n});
  options.push_back({"*", "-gaw,  --grp-attn-w N",
                     "group-attention width (default: %.1f)",
                     (double)params.grp_attn_w});
  options.push_back(
      {"*", "-dkvc, --dump-kv-cache", "verbose print of the KV cache"});
  options.push_back({"*", "-nkvo, --no-kv-offload", "disable KV offload"});
  options.push_back({"*", "-ctk,  --cache-type-k TYPE",
                     "KV cache data type for K (default: %s)",
                     params.cache_type_k.c_str()});
  options.push_back({"*", "-ctv,  --cache-type-v TYPE",
                     "KV cache data type for V (default: %s)",
                     params.cache_type_v.c_str()});
  options.push_back({"*", "-ctkd, --cache-type-k-draft TYPE",
                     "KV cache data type for K for the draft model"});
  options.push_back({"*", "-ctvd, --cache-type-v-draft TYPE",
                     "KV cache data type for V for the draft model"});

  options.push_back({"perplexity"});
  options.push_back({"perplexity", "       --all-logits",
                     "return logits for all tokens in the batch (default: %s)",
                     params.logits_all ? "true" : "false"});
  options.push_back({"perplexity", "       --hellaswag",
                     "compute HellaSwag score over random tasks from datafile "
                     "supplied with -f"});
  options.push_back({"perplexity", "       --hellaswag-tasks N",
                     "number of tasks to use when computing the HellaSwag "
                     "score (default: %zu)",
                     params.hellaswag_tasks});
  options.push_back({"perplexity", "       --winogrande",
                     "compute Winogrande score over random tasks from datafile "
                     "supplied with -f"});
  options.push_back({"perplexity", "       --winogrande-tasks N",
                     "number of tasks to use when computing the Winogrande "
                     "score (default: %zu)",
                     params.winogrande_tasks});
  options.push_back({"perplexity", "       --multiple-choice",
                     "compute multiple choice score over random tasks from "
                     "datafile supplied with -f"});
  options.push_back({"perplexity", "       --multiple-choice-tasks N",
                     "number of tasks to use when computing the multiple "
                     "choice score (default: %zu)",
                     params.multiple_choice_tasks});
  options.push_back(
      {"perplexity", "       --kl-divergence",
       "computes KL-divergence to logits provided via --kl-divergence-base"});
  options.push_back({"perplexity", "       --ppl-stride N",
  ...
  options.push_back({"*", "-mea,  --max-extra-alloc",
                     "Max extra VRAM allocation per GPU (default: %d)",
                     params.max_extra_alloc_MiB});
  options.push_back({"*", "-np,   --parallel N",
                     "number of parallel sequences to decode (default: %d)",
                     params.n_parallel});
  options.push_back({"*", "-ns,   --sequences N",
  ...
  options.push_back({"*", "       --mmproj FILE",
                     "path to a multimodal projector file for LLaVA. see "
                     "examples/llava/README.md"});
  options.push_back({"*", "       --image FILE",
                     "path to an image file. use with multimodal models. "
                     "Specify multiple times for batching"});
  options.push_back(
      {"*", "       --image-min-tokens N",
       "minimum number of tokens each image can take, only used by vision "
       "models with dynamic resolution (default: read from model)"});
  options.push_back(
      {"*", "       --image-max-tokens N",
       "maximum number of tokens each image can take, only used by vision "
       "models with dynamic resolution (default: read from model)"});
  options.push_back(
      {"*", "       --no-context-shift", "disable context-shift."});
  options.push_back({"*", "--context-shift (auto|on|off|0|1)",
                     "set context-shift (default: %s)",
                     params.ctx_shift ? "on" : "off"});
  options.push_back({"backend"});
  options.push_back(
      {"*", "       --rpc SERVERS", "comma separated list of RPC servers"});
  options.push_back(
      {"*", "-cuda, --cuda-params", "comma separate list of cuda parameters"});
  options.push_back({"*", "-draft, --draft-params",
                     "comma separate list of draft model parameters"});
  if (llama_supports_mlock()) {
    options.push_back({"*", "       --mlock",
                       "force system to keep model in RAM rather than swapping "
                       "or compressing"});
  }
  if (llama_supports_mmap()) {
    options.push_back({"*", "       --no-mmap",
                       "do not memory-map model (slower load but may reduce "
                       "pageouts if not using mlock)"});
  }
  options.push_back({"*", "       --run-time-repack",
                     "repack tensors if interleaved variant is available"});
  options.push_back(
      {"*", "       --cpu-moe", "keep all MoE weights in CPU memory"});
  options.push_back({"*", "       --n-cpu-moe N",
                     "keep MoE weights of the first N layers in CPU memory"});
  options.push_back({"*", "       --numa TYPE",
                     "attempt optimizations that help on some NUMA systems\n"
                     "  - distribute: spread execution evenly over all nodes\n"
                     "  - isolate: only spawn threads on CPUs on the node that "
                     "execution started on\n"
                     "  - numactl: use the CPU map provided by numactl\n"
                     "if run without this previously, it is recommended to "
                     "drop the system page cache before using this\n"
  ...
  if (llama_supports_gpu_offload()) {
    ...
    options.push_back({"*", "-devd,   --device-draft dev1,dev2",
                       "comma-separated list of devices to use for offloading "
                       "for the draft model (none = don't offload)\n"
                       "Example: CUDA0,CUDA1,RPC[192.168.0.1:8080]\n"});
    options.push_back(
        {"*", "-mg,   --main-gpu i",
         "the GPU to use for the model (with split-mode = none),\n"
         "or for intermediate results and KV (with split-mode = row) (default: "
         "%d)",
         params.main_gpu});
    options.push_back({"*", "--max-gpu i",
                       "max. number of GPUs to use at a time with split mode "
                       "'graph', (default: %d)",
                       params.max_gpu});
  }

  options.push_back({"model"});
  options.push_back({"*", "       --check-tensors",
  ...
  options.push_back({"*", "       --override-kv KEY=TYPE:VALUE",
                     "advanced option to override model metadata by key. may "
  ...
  options.push_back({"*", "       --lora-scaled FNAME S",
                     "apply LoRA adapter with user defined scaling S (can be "
                     "repeated to use multiple adapters)"});
  options.push_back(
  ...
  options.push_back(
      {"*", "-m,    --model FNAME",
       ...
       DEFAULT_MODEL_PATH});
  options.push_back({"*", "-md,   --model-draft FNAME",
                     "draft model for speculative decoding (default: unused)"});
  options.push_back({"*", "-mu,   --model-url MODEL_URL",
                     "model download url (default: unused)"});
  options.push_back({"*", "-hfr,  --hf-repo REPO",
                     "Hugging Face model repository (default: unused)"});
  options.push_back({"*", "-hff,  --hf-file FILE",
                     "Hugging Face model file (default: unused)"});
  options.push_back({"*", "-hft,  --hf-token TOKEN",
                     "Hugging Face access token (default: value from HF_TOKEN "
  ...
  options.push_back(
      {"*", "--draft-p-min P",
       "minimum speculative decoding probability (greedy) (default: %.1f)",
       (double)params.p_draft_min});

  options.push_back({"retrieval"});
  options.push_back(
      {"retrieval", "       --context-file FNAME",
       "file to load context from (repeat to specify multiple files)"});
  options.push_back({"retrieval", "       --chunk-size N",
  ...
  options.push_back(
      {"bench", "-ntg n0,n1,...", "number of text generation tokens"});
  options.push_back({"bench", "-npl n0,n1,...", "number of parallel prompts"});

  options.push_back({"embedding"});
  options.push_back({"embedding", "       --embd-normalize",
                     "normalisation for embendings (default: %d) (-1=none, "
                     "0=max absolute int16, 1=taxicab, 2=euclidean, >2=p-norm)",
                     params.embd_normalize});
  options.push_back(
      {"embedding", "       --embd-output-format",
       "empty = default, \"array\" = [[],[]...], \"json\" = openai style, "
       "\"json+\" = same \"json\" + cosine similarity matrix"});
  options.push_back(
      {"embedding", "       --embd-separator",
       "separator of embendings (default \\n) for example \"<#sep#>\""});

  options.push_back({"server"});
  options.push_back({"server", "       --host HOST",
                     "ip address to listen (default: %s)",
                     params.hostname.c_str()});
  options.push_back({"server", "       --port PORT",
                     "port to listen (default: %d)", params.port});
  options.push_back({"server", "       --path PATH",
                     "path to serve static files from (default: %s)",
                     params.public_path.c_str()});
  options.push_back({"server", "       --embedding(s)",
                     "restrict to only support embedding use case; use only "
                     "with dedicated embedding models (default: %s)",
  ...
  options.push_back({"server", "       --api-key KEY",
                     "API key to use for authentication (default: none)"});
  options.push_back({"server", "       --api-key-file FNAME",
                     "path to file containing API keys (default: none)"});
  options.push_back({"server", "       --ssl-key-file FNAME",
                     "path to file a PEM-encoded SSL private key"});
  options.push_back({"server", "       --ssl-cert-file FNAME",
                     "path to file a PEM-encoded SSL certificate"});
  options.push_back({"server", "       --timeout N",
                     "server read/write timeout in seconds (default: %d)",
                     params.timeout_read});
  options.push_back(
      {"server", "       --threads-http N",
  ...
  options.push_back({"server", "       --system-prompt-file FNAME",
                     "set a file to load a system prompt (initial prompt of "
                     "all slots), this is useful for chat applications"});
  ...
  options.push_back({"server", "       --no-slots",
                     "disables slots monitoring endpoint (default: %s)",
                     params.endpoint_slots ? "enabled" : "disabled"});
  options.push_back({"server", "       --slot-save-path PATH",
                     "path to save slot kv cache (default: disabled)"});
  options.push_back(
      {"server", "       --kv-recv-enable",
       "enable KV handoff receiver for prefill artifacts (default: disabled)"});
  options.push_back({"server", "       --kv-transport MODE",
                     "KV handoff transport mode: auto|rdma|tcp|mixed|disabled "
                     "(default: disabled)"});
  options.push_back({"server", "       --kv-transport-fallback",
                     "when kv-transport is rdma/tcp, allow fallback to the "
                     "other mode if unavailable (default: disabled)"});
  options.push_back({"server", "       --kv-recv-host HOST",
                     "KV receiver bind host override (default: resolved from "
                     "transport mode)"});
  options.push_back({"server", "       --kv-recv-port PORT",
                     "KV receiver bind port override (default: resolved from "
                     "transport mode)"});
  options.push_back(
      {"server", "       --kv-recv-slot ID",
       "slot ID to restore into for received KV artifacts (default: 0)"});
  options.push_back({"server", "       --kv-recv-output-dir PATH",
                     "directory for received KV chunks/artifacts (default: "
                     "slot-save-path or /tmp/ik_kv_handoff)"});
  options.push_back({"server", "       --kv-recv-dry-run",
                     "reassemble/validate incoming KV artifacts but do not "
                     "enqueue slot restore (default: disabled)"});
  options.push_back(
      {"server", "       --kv-recv-no-ack",
       "disable ACK frames for incoming KV chunks (default: ACK enabled)"});
  options.push_back({"server", "       --kv-recv-no-nack-on-crc-bad",
                     "do not send NACK when chunk payload CRC check fails "
                     "(default: enabled)"});
  options.push_back(
      {"server", "       --kv-recv-max-connections N",
       "maximum concurrent KV receiver connections (default: 32)"});
  options.push_back({"server", "       --kv-recv-idle-timeout N",
                     "idle timeout in seconds for KV receiver connection loop "
                     "(default: 30)"});
  options.push_back({"server", "       --kv-recv-stale-finalize-timeout N",
                     "force session finalization after N seconds without "
                     "frames (0 = disabled, default: 120)"});
  options.push_back({"server", "       --kv-recv-session-retention N",
                     "retain finalized sessions in memory/output dir for N "
                     "seconds before pruning (0 = disabled, default: 1800)"});
  options.push_back({"server", "       --kv-recv-cleanup-interval N",
                     "maintenance sweep interval in seconds for stale "
                     "finalization/pruning (default: 10)"});
  ...
  options.push_back({"server", "       --chat-template JINJA_TEMPLATE",
                     ...
                     "Templates-supported-by-llama_chat_apply_template"});
  options.push_back(
      {"server", "-sps,  --slot-prompt-similarity SIMILARITY",
       "how much the prompt of a request must match the prompt of a slot in "
       "order to use that slot (default: %.2f, 0.0 = disabled)\n",
       params.slot_prompt_similarity});
  options.push_back({"server", "       --lora-init-without-apply",
                     "load LoRA adapters without applying them (apply later "
                     "via POST /lora-adapters) (default: %s)",
                     params.lora_init_without_apply ? "enabled" : "disabled"});

#ifndef LOG_DISABLE_LOGS
  options.push_back({"logging"});
  options.push_back({"*", "       --simple-io",
                     "use basic IO for better compatibility in subprocesses "
                     "and limited consoles"});
  options.push_back(
      {"*", "-ld,   --logdir LOGDIR",
       "path under which to save YAML logs (no logging if unset)"});
  options.push_back(
      {"logging", "       --log-test", "Run simple logging test"});
  options.push_back({"logging", "       --log-disable", "Disable trace logs"});
  options.push_back({"logging", "       --log-enable", "Enable trace logs"});
  options.push_back({"logging", "       --log-file FNAME",
                     "Specify a log filename (without extension)"});
  options.push_back(
      {"logging", "       --log-new",
       "Create a separate new log file on start. "
       "Each log file will have unique name: \"<name>.<ID>.log\""});
  options.push_back(
      {"logging", "       --log-append", "Don't truncate the old log file."});
#endif // LOG_DISABLE_LOGS

  options.push_back({"cvector"});
  options.push_back({"cvector", "-o,    --output FNAME",
                     "output file (default: '%s')",
                     params.cvector_outfile.c_str()});
  options.push_back(
      {"cvector", "       --positive-file FNAME",
       "positive prompts file, one prompt per line (default: '%s')",
       params.cvector_positive_file.c_str()});
  options.push_back(
      {"cvector", "       --negative-file FNAME",
       "negative prompts file, one prompt per line (default: '%s')",
       params.cvector_negative_file.c_str()});
  options.push_back({"cvector", "       --pca-batch N",
                     "batch size used for PCA. Larger batch runs faster, but "
                     "uses more memory (default: %d)",
                     params.n_pca_batch});
  options.push_back({"cvector", "       --pca-iter N",
                     "number of iterations used for PCA (default: %d)",
                     params.n_pca_iterations});
  options.push_back(
      {"cvector", "       --method {pca,mean}",
       "dimensionality reduction method to be used (default: pca)"});

  options.push_back({"export-lora"});
  options.push_back({"export-lora", "-m,    --model",
                     "model path from which to load base model (default '%s')",
                     params.model.c_str()});
  options.push_back(
      {"export-lora", "       --lora FNAME",
       "path to LoRA adapter  (can be repeated to use multiple adapters)"});
  options.push_back({"export-lora", "       --lora-scaled FNAME S",
                     "path to LoRA adapter with user defined scaling S  (can "
                     "be repeated to use multiple adapters)"});
  options.push_back(
      {"*", "-t,    --threads N",
       "number of threads to use during computation (default: %d)",
       params.n_threads});
  options.push_back({"export-lora", "-o,    --output FNAME",
                     "output file (default: '%s')",
...
void string_replace_all(std::string &s, const std::string &search,
...
bool string_ends_with(const std::string_view &str,
                      const std::string_view &suffix) {
  return str.size() >= suffix.size() &&
         str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0;
}
size_t string_find_partial_stop(const std::string_view &str,
...
bool string_parse_kv_override(const char *data,
...
struct llama_init_result llama_init_from_gpt_params(gpt_params &params) {
  ...
  if (!params.hf_repo.empty() && !params.hf_file.empty()) {
    model = llama_load_model_from_hf(
        params.hf_repo.c_str(), params.hf_file.c_str(), params.model.c_str(),
        params.hf_token.c_str(), mparams);
  } else if (!params.model_url.empty()) {
  ...
  for (auto &la : params.lora_adapters) {
    ...
    iparams.lora_adapters.push_back(
        loaded_la); // copy to list of loaded adapters
  }
  ...
  if (params.warmup) {
    ...
    if (llama_model_has_decoder(model)) {
      llama_decode(
          lctx,
          llama_batch_get_one(
              tmp.data(), std::min(tmp.size(), (size_t)params.n_batch), 0, 0));
    }
...
void llama_lora_adapters_apply(
...
llama_context_params_from_gpt_params(const gpt_params &params) {
  ...
  if (!params.offload_policy.empty())
    cparams.offload_policy = (void *)&params.offload_policy;
  ...
  cparams.prefill_decode_transport_required =
      params.prefill_decode_transport_required;

  cparams.decode_remote_ranges = params.decode_remote_ranges.empty()
                                     ? nullptr
                                     : params.decode_remote_ranges.c_str();
  cparams.decode_remote_failover_policy =
      params.decode_remote_failover_policy.empty()
          ? nullptr
          : params.decode_remote_failover_policy.c_str();
  cparams.prefill_transport_session_dir =
      params.prefill_transport_session_dir.empty()
          ? nullptr
          : params.prefill_transport_session_dir.c_str();
  cparams.kv_transport =
      params.kv_transport.empty() ? nullptr : params.kv_transport.c_str();
  cparams.tb_direct_endpoint = params.tb_direct_endpoint.empty()
                                   ? nullptr
                                   : params.tb_direct_endpoint.c_str();
  ...
  cparams.kv_bind_addrs =
      params.kv_bind_addrs.empty() ? nullptr : params.kv_bind_addrs.c_str();
  cparams.kv_peer_addrs =
      params.kv_peer_addrs.empty() ? nullptr : params.kv_peer_addrs.c_str();
  cparams.kv_balance =
      params.kv_balance.empty() ? nullptr : params.kv_balance.c_str();
...
static bool llama_download_file(const std::string &url, const std::string &path,
  ...
  if (should_download) {
    ...
    std::unique_ptr<FILE, FILE_deleter> outfile(
        fopen(path_temporary.c_str(), "wb"));
    if (!outfile) {
  ...
  if (n_split > 1) {
    ...
    for (int idx = 1; idx < n_split; idx++) {
      futures_download.push_back(std::async(
          std::launch::async,
          [&split_prefix, &split_url_prefix, &n_split,
           hf_token](int download_idx) -> bool {
...
llama_load_model_from_hf(const char *repo, const char *model,
                         const char *path_model, const char *hf_token,
                         const struct llama_model_params &params) {
  ...
  return llama_load_model_from_url(model_url.c_str(), path_model, hf_token,
                                   params);
}
...
struct llama_model *llama_load_model_from_url(
    const char * /*model_url*/, const char * /*path_model*/,
    const char * /*hf_token*/, const struct llama_model_params & /*params*/) {
  fprintf(stderr,
...
std::vector<llama_token> llama_tokenize(const struct llama_context *ctx,
  ...
  return common_tokenize(llama_get_model(ctx), text, add_special,
                         parse_special);
}

std::vector<llama_token> common_tokenize(const struct llama_model *model,
                                         const std::string &text,
                                         bool add_special, bool parse_special) {
...
std::vector<llama_token> llama_tokenize(const struct llama_vocab *vocab,
                                        const std::string &text,
                                        bool add_special, bool parse_special) {
  // upper limit for the number of tokens
  int n_tokens = text.length() + 2 * add_special;
  std::vector<llama_token> result(n_tokens);
  n_tokens =
      llama_vocab_tokenize(vocab, text.data(), text.length(), result.data(),
                           result.size(), add_special, parse_special);
...
std::string common_token_to_piece(const struct llama_context *ctx,
  ...
  const int n_chars = llama_token_to_piece(llama_get_model(ctx), token,
                                           &piece[0], piece.size(), 0, special);
  if (n_chars < 0) {
    piece.resize(-n_chars);
    int check = llama_token_to_piece(llama_get_model(ctx), token, &piece[0],
                                     piece.size(), 0, special);
...
std::string llama_token_to_piece(const struct llama_model *model,
                                 llama_token token, bool special) {
  std::string piece;
  piece.resize(
      piece.capacity()); // using string internal cache, 15 bytes + '\n'
  const int n_chars =
      llama_token_to_piece(model, token, &piece[0], piece.size(), 0, special);
  if (n_chars < 0) {
    piece.resize(-n_chars);
    int check =
        llama_token_to_piece(model, token, &piece[0], piece.size(), 0, special);
...
std::string common_token_to_piece(const llama_context *ctx,
                                  const std::vector<llama_token> &tokens,
                                  bool special) {
  std::string text;
  text.resize(std::max(text.capacity(), tokens.size()));
  int32_t n_chars = llama_detokenize(llama_get_model(ctx), tokens.data(),
                                     (int32_t)tokens.size(), &text[0],
                                     (int32_t)text.size(), false, special);
  if (n_chars < 0) {
    text.resize(-n_chars);
    n_chars = llama_detokenize(llama_get_model(ctx), tokens.data(),
                               (int32_t)tokens.size(), &text[0],
                               (int32_t)text.size(), false, special);
    GGML_ASSERT(n_chars <=
                (int32_t)text.size()); // whitespace trimming is performed after
                                       // per-token detokenization
  }
...
void llama_kv_cache_dump_view_seqs(const llama_kv_cache_view &view,
                                   int row_size) {
  static const char slot_chars[] =
...
float common_embd_similarity_cos(const float *embd1, const float *embd2,
                                 int n) {
  double sum = 0.0;
  double sum1 = 0.0;
  double sum2 = 0.0;
...
llama_control_vector_load_one(const llama_control_vector_load_info &load_info) {
  ...
  for (int i = 0; i < n_tensors; i++) {
    ...
    result.data.resize(std::max(result.data.size(),
                                static_cast<size_t>(result.n_embd * layer_idx)),
                       0.0f);

    const float *src = (const float *)tensor->data;
    float *dst =
        result.data.data() + result.n_embd * (layer_idx - 1); // layer 1 at [0]
    for (int j = 0; j < result.n_embd; j++) {
      dst[j] += src[j] * load_info.strength; // allows multiple directions for
...
void yaml_dump_vector_float(FILE *stream, const char *prop_name,
...
void yaml_dump_vector_int(FILE *stream, const char *prop_name,
...
void yaml_dump_string_multiline(FILE *stream, const char *prop_name,
                                const char *data) {
...
void yaml_dump_non_result_info(FILE *stream, const gpt_params &params,
  ...
  yaml_dump_string_multiline(stream, "cfg_negative_prompt",
                             sparams.cfg_negative_prompt.c_str());
  ...
  fprintf(
      stream,
      "dry_penalty_last_n: %d # default: -1 (0 = disable, -1 = context size)\n",
