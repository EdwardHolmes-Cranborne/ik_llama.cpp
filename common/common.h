...
enum common_reasoning_format {
  ...
  COMMON_REASONING_FORMAT_DEEPSEEK_LEGACY, // Extract thinking tag contents and
                                           // return as
                                           // `message.reasoning_content`, or
                                           // leave inline in <think> tags in
                                           // stream mode
  COMMON_REASONING_FORMAT_DEEPSEEK, // Extract thinking tag contents and return
                                    // as `message.reasoning_content`, including
                                    // in streaming deltas.
};
...
struct gpt_params {
  ...
  int32_t n_draft_min =
      1; // minimum number of tokens to draft during speculative decoding
  ...
  int32_t max_gpu =
      0; // max number of GPUs to use at a time for split mode "graph"
  ...
  int32_t max_extra_alloc_MiB =
      256; // additional VRAM per GPU the scheduler may allocate for more
           // efficient compute graph evaluation
  ...
  bool prompt_is_binary = false; // don't fool around when the prompt contains
                                 // binary data (as it is for multiple choice)
  ...
  std::string cuda_params =
      ""; // comma separated list of cuda parameters key=value1,key2=value2

  std::vector<std::string> in_files;    // all input files
  std::vector<std::string> antiprompt;  // strings upon which more user input is
                                        // prompted (a.k.a. reverse prompts)
  ...
  std::vector<std::pair<std::string, std::string>>
      replacements_draft; // main to speculative model replacements

  bool lora_init_without_apply =
      false; // only load lora to memory, but do not apply it to ctx (user can
             // manually apply lora later using llama_lora_adapter_apply)
  std::vector<llama_lora_adapter_info>
  ...
  int32_t ppl_stride = 0; // stride for perplexity calculations. If left at 0,
                          // the pre-existing approach will be used.
  ...
  int mla_attn = 3; // MLA 0: standard, 1: MLA with K and V^T cache, 2: MLA with
                    // just K cache, 3: the best of both worlds
  int attn_max_batch = 0; // Max batch size to use when computing attention
                          // (only applicable if flash_attn = false)
  ...
  bool grouped_expert_routing =
      false; // if to use grouped expert routing (BailingMoeV2 arch)
  ...
  bool repack_tensors =
      false;            // repack tensors if interleaved variant is available
  bool use_thp = false; // use transparent huge pages (linux only)
  bool validate_quants =
      false; // if true, check for NaNs while loading the model
  bool only_active_exps = true; // if true, offload only active experts
                                // (relevant only for hybrid CPU/GPU)
  bool merge_qkv = false; // if true, merge separate Q, K, V tensors into a
                          // single, contiguous tensor
  bool merge_up_gate_exps =
      false; // if true, merge ffn_up_exps and ffn_gate_exps into a single,
             // contiguous tensor
  bool k_cache_hadamard =
      false; // if true, use Hadamard transform for the K-cache (only makes
             // sense with quantized cache)
  bool split_mode_graph_scheduling =
      false; // if true, force split mode graph scheduling
  // bool split_mode_f16    = true;  // if true, intermediate results will be
  // cast to f16 before copying to other GPUs to perform reduce ops
  bool scheduler_async =
      false; // if true, in split mode graph the scheduler will use multiple
             // threads to evaluate the graph

  std::string cache_type_k = "f16"; // KV cache data type for the K
  std::string cache_type_v = "f16"; // KV cache data type for the V
  std::string cache_type_k_draft =
      ""; // KV cache data type for K for the draft model
  std::string cache_type_v_draft =
  ...
  int32_t embd_normalize =
      2; // normalisation for embendings (-1=none, 0=max absolute int16,
  ...
  int32_t cache_ram_n_min =
      0; // min number of tokens required to save in the ram
  ...
  // imatrix params
  std::string out_file =
      "imatrix.dat"; // save the resulting imatrix to this file
...
static bool string_starts_with(
    ...
    const std::string
        &prefix) { // While we wait for C++20's std::string::starts_with...
...
template <>
std::vector<std::string> string_split<std::string>(const std::string &input,
                                                   char separator) {
...
void llama_kv_cache_dump_view(const llama_kv_cache_view &view,
                              int row_size = 80);
...
void common_embd_normalize(const float *inp, float *out, int n,
                           int embd_norm = 2);
